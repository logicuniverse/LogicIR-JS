...
edits describe useful workspace policy; other edits only make sense if the
...
| `docs/workspace/README.md` | Add `explorations/` as a workspace category for unconfirmed automatic drafts and staged discussion material. See Directly Useful Workspace Policy. | Already reintroduced in a narrower form by this exploration work. |
...
## Directly Useful Workspace Policy

These points are useful independent of whether the archived automatic-run code
is reintroduced:

- Add an `explorations/` workspace concept for stage results that are useful
...
can later be copied into stable workspace docs if the human confirms them.
...
- Turn this roadmap into a reviewed plan under `docs/workspace/plans/` before
