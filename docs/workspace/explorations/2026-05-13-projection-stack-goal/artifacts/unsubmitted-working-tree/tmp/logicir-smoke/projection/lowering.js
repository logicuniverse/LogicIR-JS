...
exports.lowerJSRuntimeSkeleton = lowerJSRuntimeSkeleton;
exports.lowerJSExecutableRuntime = lowerJSExecutableRuntime;
exports.lowerPythonRuntimeSkeleton = lowerPythonRuntimeSkeleton;
exports.lowerPythonExecutableRuntime = lowerPythonExecutableRuntime;
exports.lowerVerilogModuleSkeleton = lowerVerilogModuleSkeleton;
function lowerJSRuntimeSkeleton(plan) {
    return {
        kind: 'source',
        language: 'javascript',
...
function lowerJSExecutableRuntime(plan) {
    return {
        kind: 'source',
        language: 'javascript',
...
function lowerPythonRuntimeSkeleton(plan) {
    return {
        kind: 'source',
        language: 'python',
...
function lowerPythonExecutableRuntime(plan) {
    return {
        kind: 'source',
        language: 'python',
...
function lowerVerilogModuleSkeleton(plan, moduleName = 'logicir_top') {
    const rootSignals = plan.signals.filter((signal) => signal.ownerPath.length >= 3 &&
        signal.ownerPath[0] === 'core' &&
        signal.ownerPath[1] === 'ports');
    const portNames = rootSignals.map((signal) => signalVerilogName(signal));
    const lines = [];
    const signalMap = new Map(plan.signals.map((signal) => [
    ...
    lines.push(portNames.map((portName) => `  ${portName}`).join(',\n'));
    lines.push(');');
    lines.push('');
    for (const signal of rootSignals) {
        lines.push(formatVerilogPort(signal));
    }
    const internalSignals = plan.signals.filter((signal) => !(signal.ownerPath.length >= 3 &&
        signal.ownerPath[0] === 'core' &&
        signal.ownerPath[1] === 'ports'));
    const stateRegisterTargets = new Set(collectStateRegisterTargetKeys(plan.stateRegisters));
    if (internalSignals.length > 0) {
        lines.push('');
        lines.push('  // Planned internal LUI/closure boundary wires.');
        for (const signal of internalSignals) {
            lines.push(stateRegisterTargets.has(endpointSignalKey(signalToEndpoint(signal)))
                ? formatVerilogReg(signal)
                : formatVerilogWire(signal));
        }
    }
    const instances = formatVerilogInstances(plan.core, plan.moduleBindings, plan.clockReset, signalMap);
    if (instances.length > 0) {
        lines.push('');
        lines.push('  // Planned external/LU module instances.');
        lines.push(...instances);
    }
    const structuralSliceWires = formatVerilogStructuralSliceInterfaceWires(plan.structuralSlicePlans);
    const structuralSliceInstances = formatVerilogStructuralSliceInstances(plan.structuralSlicePlans);
    ...
    const structuralSliceLinks = formatVerilogStructuralSliceLinks(plan.structuralSliceLinks, plan.structuralSlicePlans);
    if (structuralSliceLinks.length > 0) {
        lines.push('');
        lines.push('  // Planned structural slice cross-links.');
        lines.push(...structuralSliceLinks);
    }
    const structuralSlices = formatVerilogStructuralSlices(plan.structuralSlicePlans);
    ...
    const behaviorAssigns = formatVerilogCombinationalAssigns(plan.combinationalAssigns, signalMap);
    if (behaviorAssigns.length > 0) {
        lines.push('');
        lines.push('  // Planned combinational behavior.');
        lines.push(...behaviorAssigns);
    }
    const stateRegisterBlocks = formatVerilogStateRegisters(plan.stateRegisters, plan.clockReset, signalMap);
    ...
    lines.push('');
    const sliceModules = formatVerilogStructuralSliceModules(plan.structuralSlicePlans);
...
function summarizeCore(core) {
    return {
        ...
        luis: Object.fromEntries(Object.entries(core.luis).map(([luiId, lui]) => [
            luiId,
            {
                kind: lui.kind,
                target: lui.target,
...
function summarizePolicies(policies) {
    return Object.fromEntries(Object.entries(policies).map(([key, values]) => [key, values.length]));
}
function pythonJsonLoadsExpression(value) {
    return `json.loads(${JSON.stringify(JSON.stringify(value, null, 2))})`;
}
function formatVerilogPort(signal) {
...
function formatVerilogWire(signal) {
    const signed = signal.signed ? ' signed' : '';
    const range = signal.width === 1 ? '' : ` [${signal.width - 1}:0]`;
    return `  wire${signed}${range} ${signalVerilogName(signal)};`;
}
function formatVerilogReg(signal) {
    const signed = signal.signed ? ' signed' : '';
    const range = signal.width === 1 ? '' : ` [${signal.width - 1}:0]`;
    return `  reg${signed}${range} ${signalVerilogName(signal)};`;
}
function formatClockResetNotes(clockReset) {
    const notes = [];
    for (const extension of clockReset) {
        ...
        const domain = typeof payload.domain === 'string' ? payload.domain : 'default';
        const clockPort = typeof payload.clock.portKey === 'string'
            ? payload.clock.portKey
            : 'clock';
        const edge = payload.clock.edge === 'negedge' ? 'negedge' : 'posedge';
        const reset = isRecord(payload.reset)
            ? ` reset=${String(payload.reset.portKey)} ${String(payload.reset.kind)} active-${String(payload.reset.active)}`
            : '';
        notes.push(`// HDL clock-reset domain ${sanitizeVerilogComment(domain)}: ${edge} ${sanitizeVerilogComment(clockPort)}${sanitizeVerilogComment(reset)}`);
    }
    return notes;
}
function formatVerilogStructuralSlices(slices) {
    const lines = [];
    for (const slice of slices) {
        ...
        const placement = slice.placement
            ...
            : '';
        lines.push(`  // slice ${sanitizeVerilogComment(slice.anchorKey)} -> module ${sanitizeVerilogName(slice.moduleName)} (${required})${placement}`);
        if (slice.rxPort || slice.txPort) {
            lines.push(`  //   bus rx=${sanitizeVerilogComment(slice.rxPort ?? '-')}, tx=${sanitizeVerilogComment(slice.txPort ?? '-')}`);
        }
        if (slice.rx || slice.tx) {
            lines.push(`  //   interface rx=${sanitizeVerilogComment(formatSliceInterfaceSummary(slice.rx))}, tx=${sanitizeVerilogComment(formatSliceInterfaceSummary(slice.tx))}`);
        }
        if (slice.bus) {
            const channelPath = slice.bus.channelPath && slice.bus.channelPath.length > 0
                ? ` channelPath=${slice.bus.channelPath.map(String).join('.')}`
                : '';
            lines.push(`  //   routing=${sanitizeVerilogComment(slice.bus.routing)}${sanitizeVerilogComment(channelPath)}`);
        }
        lines.push(`  //   root=${sanitizeVerilogComment(formatCompositionLeaf(slice.root))}`);
        if (slice.footprint.luis.length > 0) {
            lines.push(`  //   footprint.luis=${sanitizeVerilogComment(slice.footprint.luis.join(','))}`);
        }
        if (slice.footprint.luiOutlets.length > 0) {
            lines.push(`  //   footprint.luiOutlets=${sanitizeVerilogComment(slice.footprint.luiOutlets
                .map((ref) => `${ref.luiId}.${ref.outletKey}`)
                .join(','))}`);
        }
        if (slice.footprint.externalOutlets.length > 0) {
            lines.push(`  //   footprint.externalOutlets=${sanitizeVerilogComment(slice.footprint.externalOutlets.join(','))}`);
        }
        if (slice.footprint.anchors.length > 0) {
            lines.push(`  //   footprint.anchors=${sanitizeVerilogComment(slice.footprint.anchors
                .map((ref) => `${ref.luiId}.${ref.anchorKey}`)
                .join(','))}`);
        }
        const childFills = Object.entries(slice.childAnchorFills);
        if (childFills.length > 0) {
            lines.push(`  //   childAnchorFills=${sanitizeVerilogComment(childFills
...
function formatVerilogStructuralSliceModules(slices) {
    const lines = [];
    for (const slice of slices) {
        const ports = collectSliceInterfacePorts(slice);
        const moduleName = sanitizeVerilogName(slice.moduleName);
        lines.push(`// Structural slice stub for export anchor ${sanitizeVerilogComment(slice.anchorKey)}.`);
        lines.push(`module ${moduleName}${formatVerilogPortList(ports.map((port) => port.name))};`);
...
function formatVerilogStructuralSliceInterfaceWires(slices) {
    const lines = [];
    const seen = new Set();
    for (const slice of slices) {
...
function formatVerilogStructuralSliceInstances(slices) {
    const lines = [];
    for (const slice of slices) {
        const ports = collectSliceInterfacePorts(slice);
        if (ports.length === 0) {
        ...
        lines.push(ports
            .map((port) => `    .${sanitizeVerilogName(port.name)}(${sliceInterfaceWireName(slice, port.name)})`)
            .join(',\n'));
        lines.push('  );');
    }
    return lines;
}
function formatVerilogStructuralSliceLinks(links, slices) {
    const byAnchor = new Map(slices.map((slice) => [structuralSliceKey(slice.corePath, slice.anchorKey), slice]));
    const lines = [];
    ...
    for (const [consumerKey, consumerLinks] of linksByConsumer) {
        ...
        const providers = consumerLinks
            .map((link) => {
            const provider = byAnchor.get(structuralSliceKey(link.corePath, link.fromAnchorKey));
        ...
        if (providers.length > 1 && consumer.fanIn) {
            const op = consumer.fanIn.policy === 'and'
                ? '&'
                : consumer.fanIn.policy === 'xor'
                    ? '^'
                    : '|';
            lines.push(`  // fan-in ${sanitizeVerilogComment(consumer.fanIn.policy)} -> ${sanitizeVerilogComment(consumer.anchorKey)}`);
            lines.push(`  assign ${sliceInterfaceWireName(consumer, rxPortName)} = ${providers
                .map((provider) => sliceInterfaceWireName(provider.provider, provider.txPortName))
                .join(` ${op} `)};`);
            continue;
        }
        const [single] = providers;
        lines.push(`  // ${sanitizeVerilogComment(single.link.fromAnchorKey)}.${sanitizeVerilogComment(single.link.luiId)}.${sanitizeVerilogComment(single.link.outletKey)} -> ${sanitizeVerilogComment(single.link.toAnchorKey)}`);
        lines.push(`  assign ${sliceInterfaceWireName(consumer, rxPortName)} = ${sliceInterfaceWireName(single.provider, single.txPortName)};`);
    }
    return lines;
}
function structuralSliceKey(corePath, anchorKey) {
    return `${JSON.stringify(corePath)}:${anchorKey}`;
}
function formatVerilogSliceInterfaceWire(wireName, signal) {
    const signed = signal?.signed ? ' signed' : '';
    const width = signal?.width ?? 1;
    const range = width > 1 ? ` [${width - 1}:0]` : '';
    return `  wire${signed}${range} ${wireName};`;
}
function sliceInterfaceWireName(slice, portName) {
    return sanitizeVerilogName(`slice_${slice.anchorKey}_${slice.moduleName}_${portName}`);
}
function collectSliceInterfacePorts(slice) {
    const ports = [];
    const names = new Set();
    const push = (name, direction, signal) => {
...
function formatVerilogSliceInterfacePort(port) {
    ...
    const signed = signal?.signed ? ' signed' : '';
    const range = signal && signal.width > 1 ? ` [${signal.width - 1}:0]` : '';
    return `  ${port.direction}${signed}${range} ${sanitizeVerilogName(port.name)};`;
}
function formatSliceInterfaceSummary(signal) {
    if (!signal) {
        return '-';
    }
    const signed = signal.signed ? ' signed' : '';
...
function formatVerilogPortList(ports) {
    if (ports.length === 0) {
        return '';
    }
    return `(\n${ports
...
function formatCompositionLeaf(leaf) {
    if (!leaf) {
        return 'missing';
    }
    if (leaf.kind === 'empty') {
...
function formatVerilogAssign(connection, signalMap) {
    ...
    const fromExpression = formatPackedAggregateSource(connection.from, from, signalMap) ??
        signalVerilogName(from);
    const assigns = [
        `  assign ${signalVerilogName(to)} = ${fromExpression};`,
    ];
    const packedAggregateTarget = formatPackedAggregateTarget(connection.to, to, signalMap);
    if (packedAggregateTarget) {
        assigns.push(`  assign ${packedAggregateTarget} = ${fromExpression};`);
    }
    return assigns;
}
function formatPackedAggregateSource(endpoint, pathSignal, signalMap) {
    const partSelect = formatPackedAggregateEndpoint(endpoint, pathSignal, signalMap);
    return partSelect;
}
function formatPackedAggregateTarget(endpoint, pathSignal, signalMap) {
    return formatPackedAggregateEndpoint(endpoint, pathSignal, signalMap);
}
function formatPackedAggregateEndpoint(endpoint, pathSignal, signalMap) {
    const rootSignal = signalMap.get(endpointSignalKey({ ...endpoint, payloadPath: undefined }));
    if (!rootSignal ||
        !rootSignal.packed ||
        !pathSignal.packed ||
...
function formatVerilogCombinationalAssigns(extensions, signalMap) {
    const lines = [];
...
function collectStateRegisterTargetKeys(extensions) {
    const keys = [];
    for (const extension of extensions) {
        const payload = extension.payload;
...
function formatVerilogStateRegisters(extensions, clockReset, signalMap) {
    const lines = [];
    for (const extension of extensions) {
        const payload = extension.payload;
        if (!isRecord(payload) || !Array.isArray(payload.registers)) {
        ...
        for (const register of payload.registers) {
            ...
            const domain = selectClockResetForRegister(typeof register.clockResetDomain === 'string'
                ? register.clockResetDomain
                : undefined, clockReset);
            if (!domain || !isRecord(domain.payload) || !isRecord(domain.payload.clock)) {
                lines.push('  // state register missing clock-reset domain');
                continue;
            }
            const clockPortKey = typeof domain.payload.clock.portKey === 'string'
                ? domain.payload.clock.portKey
                : undefined;
            const clockSignal = clockPortKey
                ? signalMap.get(endpointSignalKey({ owner: { kind: 'lu' }, portKey: clockPortKey }))
                : undefined;
            if (!clockSignal) {
            ...
            const resetPortKey = reset && typeof reset.portKey === 'string' ? reset.portKey : undefined;
            const resetSignal = resetPortKey
                ? signalMap.get(endpointSignalKey({ owner: { kind: 'lu' }, portKey: resetPortKey }))
                : undefined;
            ...
            if (reset && resetSignal && resetValue) {
                const condition = reset.active === 'low'
...
function selectClockResetForRegister(domain, clockReset) {
    if (!domain) {
        return clockReset[0];
    }
...
function formatVerilogExpression(expr, signalMap) {
    if (!isRecord(expr)) {
        return undefined;
    }
    ...
    if (expr.kind === 'cast') {
        ...
        const signed = expr.signed === true ? '$signed' : expr.signed === false ? '$unsigned' : undefined;
...
function formatVerilogConstant(value, width) {
...
function formatSizedConstant(value, width) {
    ...
    return (isRecord(value) &&
        ...
        (value.payloadPath === undefined ||
            (Array.isArray(value.payloadPath) &&
                value.payloadPath.every((segment) => typeof segment === 'string' ||
                    (typeof segment === 'number' && Number.isInteger(segment))))));
}
function isVerilogUnaryOp(value) {
    return value === '~' || value === '!' || value === '-';
}
function isVerilogReductionOp(value) {
    return (value === '&' ||
        value === '|' ||
        value === '^' ||
        value === '~&' ||
...
function isVerilogBinaryOp(value) {
    return (value === '+' ||
        value === '-' ||
        value === '*' ||
        value === '&' ||
...
function formatVerilogInstances(core, moduleBindings, clockReset, signalMap) {
    const lines = [];
    for (const [luiId, lui] of Object.entries(core.luis)) {
        ...
        const instanceName = typeof payload.instanceName === 'string'
        ...
        const ports = Object.keys(lui.ports).map((portKey) => {
            const externalName = typeof explicitPortMap[portKey] === 'string'
            ...
            const signal = signalMap.get(endpointSignalKey(endpoint)) ??
        ...
        ports.push(...formatVerilogClockResetInstancePorts(luiId, lui.kind, explicitPortMap, clockReset, signalMap, new Set(Object.keys(lui.ports))));
        lines.push(`  ${sanitizeVerilogName(payload.moduleName)}${parameters} ${sanitizeVerilogName(instanceName)} (`);
        lines.push(ports.join(',\n'));
        lines.push('  );');
    }
    return lines;
}
function formatVerilogClockResetInstancePorts(luiId, luiKind, explicitPortMap, clockReset, signalMap, existingPortKeys) {
    const domain = selectClockResetForLUI(luiId, luiKind, clockReset);
    if (!domain || !isRecord(domain.payload) || !isRecord(domain.payload.clock)) {
        return [];
    }
    const ports = [];
    const addRootPort = (portKey) => {
        if (typeof portKey !== 'string' || existingPortKeys.has(portKey)) {
            return;
        }
        const rootSignal = signalMap.get(endpointSignalKey({ owner: { kind: 'lu' }, portKey }));
        if (!rootSignal) {
            return;
        }
        const externalName = typeof explicitPortMap[portKey] === 'string'
        ...
        ports.push(`    .${sanitizeVerilogName(externalName)}(${signalVerilogName(rootSignal)})`);
    };
...
function selectClockResetForLUI(luiId, luiKind, clockReset) {
    const directlyAttached = clockReset.find((extension) => pathSelectsLUI(extension.path, luiId));
    if (directlyAttached) {
...
function pathSelectsLUI(path, luiId) {
    return path.some((segment, index) => segment === 'luis' &&
        path[index + 1] === luiId);
}
function findFirstPortSignal(signalMap, endpoint) {
...
function formatVerilogParameters(parameters) {
    const entries = Object.entries(parameters);
    if (entries.length === 0) {
        return '';
    }
    return ` #(\n${entries
        .map(([key, value]) => `    .${sanitizeVerilogName(key)}(${formatVerilogParameterValue(value)})`)
        .join(',\n')}\n  )`;
}
function formatVerilogParameterValue(value) {
...
function formatEndpoint(endpoint) {
    const owner = endpoint.owner.kind === 'lu'
        ? 'lu'
        : endpoint.owner.kind === 'lui'
            ? `lui:${endpoint.owner.luiId}`
            : `closure:${endpoint.owner.closureId}`;
    const path = endpoint.payloadPath && endpoint.payloadPath.length > 0
...
function signalVerilogName(signal) {
    ...
    if (signal.ownerPath[1] === 'luis' && typeof signal.ownerPath[2] === 'string') {
        return sanitizeVerilogName(`lui_${signal.ownerPath[2]}_${signal.portKey}${pathSuffix}`);
    }
    if (signal.ownerPath[1] === 'closures' &&
        typeof signal.ownerPath[2] === 'string') {
        return sanitizeVerilogName(`closure_${signal.ownerPath[2]}_${signal.portKey}${pathSuffix}`);
    }
    return sanitizeVerilogName(`${signal.portKey}${pathSuffix}`);
}
function signalPathSuffix(signal) {
    if (!signal.payloadPath || signal.payloadPath.length === 0) {
        return '';
    }
    return `_${signal.payloadPath.map(String).join('_')}`;
}
function sanitizeVerilogName(value) {
...
function sanitizeVerilogComment(value) {
