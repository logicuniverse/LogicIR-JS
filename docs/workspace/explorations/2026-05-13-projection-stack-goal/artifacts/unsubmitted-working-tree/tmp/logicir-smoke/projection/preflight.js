...
exports.preflightProjection = preflightProjection;
const validator_1 = require("../core/v0-draft/validator");
const validator_2 = require("../extensions/drafts/validator");
function preflightProjection(unit, capabilities, options = {}) {
    const diagnostics = [];
    const includeCoreValidation = options.includeCoreValidation ?? true;
    const add = (code, path, message, severity = 'error') => {
        diagnostics.push({ code, path, message, severity });
    };
    if (includeCoreValidation) {
        const coreDiagnostics = (0, validator_1.validateLogicUnit)(unit, {
            capabilities,
        ...
        coreDiagnostics.forEach((diagnostic) => {
            add(diagnostic.rule, diagnostic.path, diagnostic.message, diagnostic.severity);
        });
    }
    validateCoreCapabilities(unit, capabilities, add);
    diagnostics.push(...(0, validator_2.validateDraftExtensions)(unit, {
        capabilities,
        resolveLogicUnit: options.resolveLogicUnit,
        resolveRequirementService: options.resolveRequirementService,
    }));
...
function validateCoreCapabilities(unit, capabilities, add) {
...
function validateCore(core, capabilities, path, add) {
    ...
    for (const [luiId, lui] of luiEntries(core)) {
        ...
        for (const [serviceKey, fulfillment] of entries(lui.fulfillments)) {
            if (!capabilities.core.fulfillmentScopes.includes(fulfillment.kind)) {
                add('PRJ-004', [...luiPath, 'fulfillments', serviceKey, 'kind'], `Projector does not support '${fulfillment.kind}' requirement fulfillment.`);
            }
            if (fulfillment.kind === 'independent-units' &&
                ...
                for (const [unitKey, unitFulfillment] of entries(fulfillment.units)) {
                    if (unitFulfillment.kind === 'closure') {
                        add('PRJ-005', [...luiPath, 'fulfillments', serviceKey, 'units', unitKey], 'Projector does not support closure fulfillment.');
                    }
                }
            }
            if (fulfillment.kind === 'independent-units' &&
                ...
                for (const [unitKey, unitFulfillment] of entries(fulfillment.units)) {
                    if (unitFulfillment.kind === 'upstream-unit') {
                        add('PRJ-006', [...luiPath, 'fulfillments', serviceKey, 'units', unitKey], 'Projector does not support upstream unit fulfillment.');
                    }
                }
            }
            if (fulfillment.kind === 'shared-service' &&
                !capabilities.core.requirements.upstreamSharedServiceFulfillment) {
                add('PRJ-007', [...luiPath, 'fulfillments', serviceKey], 'Projector does not support upstream shared-service fulfillment.');
            }
        }
        if (lui.kind === 'structural') {
            if (!capabilities.core.structuralComposition.childOutlets) {
                add('PRJ-008', [...luiPath, 'compositionSurface', 'outlets'], 'Projector does not support structural child outlets.');
            }
            if (!capabilities.core.structuralComposition.childAnchors) {
                add('PRJ-009', [...luiPath, 'compositionSurface', 'anchors'], 'Projector does not support structural child anchors.');
            }
            for (const [anchorKey, anchor] of entries(lui.compositionSurface.anchors)) {
                if (anchor.shape === 'collection' && !capabilities.core.structuralComposition.collections) {
                    add('PRJ-010', [...luiPath, 'compositionSurface', 'anchors', anchorKey], 'Projector does not support collection composition anchors.');
                }
                if (anchor.shape === 'map' && !capabilities.core.structuralComposition.maps) {
                    add('PRJ-011', [...luiPath, 'compositionSurface', 'anchors', anchorKey], 'Projector does not support map composition anchors.');
                }
            }
        }
    }
    for (const [connectionId, connection] of entries(core.connections)) {
        for (const side of ['from', 'to']) {
            ...
            if (endpoint.payloadPath &&
                endpoint.payloadPath.length > 0 &&
                !capabilities.core.endpointAddressing.payloadPath) {
                add('PRJ-012', [...path, 'connections', connectionId, side, 'payloadPath'], 'Projector does not support endpoint payloadPath addressing.');
            }
            if (endpoint.payloadPath &&
                ...
                add('PRJ-013', [...path, 'connections', connectionId, side, 'payloadPath'], 'Endpoint payloadPath exceeds projector maximum depth.');
            }
        }
    }
    if (core.kindOrganization.kind === 'structural') {
        if (!capabilities.core.structuralComposition.exportAnchors) {
            add('PRJ-014', [...path, 'kindOrganization', 'exportAnchors'], 'Projector does not support structural export anchors.');
        }
        if (Object.keys(core.kindOrganization.externalOutlets).length > 0 &&
            !capabilities.core.structuralComposition.externalOutlets) {
            add('PRJ-015', [...path, 'kindOrganization', 'externalOutlets'], 'Projector does not support structural external outlets.');
        }
        for (const [anchorKey, value] of entries(core.kindOrganization.luiFills)) {
            validateCompositionValueCapabilities(value, capabilities, [...path, 'kindOrganization', 'luiFills', anchorKey], add);
        }
    }
...
function validateTargetProjectionConstraints(core, capabilities, path, add) {
    ...
    for (const [closureId, closure] of entries(core.closures)) {
        validateTargetProjectionConstraints(closure.core, capabilities, [...path, 'closures', closureId, 'core'], add);
    }
}
function validateVerilogCoreConstraints(core, hdl, path, add) {
    for (const [portKey, port] of entries(core.ports)) {
        validateVerilogPortConstraints(port, hdl, [...path, 'ports', portKey], add);
    }
    for (const [luiId, lui] of luiEntries(core)) {
        for (const [portKey, port] of entries(lui.ports)) {
            validateVerilogPortConstraints(port, hdl, [...path, 'luis', luiId, 'ports', portKey], add);
        }
    }
}
function validateVerilogRuntimeExtensionConstraints(core, path, add) {
    validateVerilogRuntimeExtensionList(core.extensions, [...path, 'extensions'], add);
    for (const [portKey, port] of entries(core.ports)) {
        validateVerilogRuntimeExtensionList(port.extensions, [...path, 'ports', portKey, 'extensions'], add);
    }
    for (const [luiId, lui] of luiEntries(core)) {
        const luiPath = [...path, 'luis', luiId];
        validateVerilogRuntimeExtensionList(lui.extensions, [...luiPath, 'extensions'], add);
        for (const [portKey, port] of entries(lui.ports)) {
            validateVerilogRuntimeExtensionList(port.extensions, [...luiPath, 'ports', portKey, 'extensions'], add);
        }
    }
}
function validateVerilogRuntimeExtensionList(extensions, path, add) {
    extensions?.forEach((extension, index) => {
        const isRuntimeDynamicFulfillment = (extension.feature.namespace === 'logicir.js-runtime' ||
            extension.feature.namespace === 'logicir.python-runtime') &&
            extension.key === 'dynamic-fulfillment';
        if (!isRuntimeDynamicFulfillment) {
            return;
        }
        add('PRJ-027', [...path, index], 'Verilog HDL projection cannot realize software runtime dynamic fulfillment policy.', extension.requirement === 'required' ? 'error' : 'warning');
    });
}
function validateVerilogPortConstraints(port, hdl, path, add) {
    if (port.interaction.pushNotifiable && !hdl.supportsPushNotifiable) {
        add('PRJ-025', [...path, 'interaction', 'pushNotifiable'], 'Verilog HDL capability does not support push-notifiable contacts.');
    }
    if (port.interaction.retainedCurrent && !hdl.supportsRetainedCurrent) {
        add('PRJ-026', [...path, 'interaction', 'retainedCurrent'], 'Verilog HDL capability does not support retained-current contacts.');
    }
}
function validatePortCapability(port, capabilities, path, add) {
    ...
    if (interaction.combinations) {
        const matched = interaction.combinations.some((combination) => combination.pullReadable === port.interaction.pullReadable &&
...
function validateCompositionValueCapabilities(value, capabilities, path, add) {
    for (const [key, item] of entries(value)) {
        if (item.kind === 'collection') {
            if (!capabilities.core.structuralComposition.collections) {
                add('PRJ-010', [...path, key], 'Projector does not support collection composition values.');
            }
            continue;
        }
        if (item.kind === 'map' && !capabilities.core.structuralComposition.maps) {
            add('PRJ-011', [...path, key], 'Projector does not support map composition values.');
        }
    }
}
function validateRequirementSurfaceCapabilities(surface, capabilities, path, add) {
    for (const [serviceKey, entry] of entries(surface)) {
        if (entry.kind === 'external') {
            if (!capabilities.core.requirements.externalServices) {
                add('PRJ-021', [...path, serviceKey], 'Projector does not support external requirement services.');
            }
            continue;
        }
        if (!capabilities.core.requirements.inlineServices) {
            add('PRJ-022', [...path, serviceKey], 'Projector does not support inline requirement services.');
        }
        if (!capabilities.core.fulfillmentScopes.includes(entry.service.fulfillmentScope)) {
            add('PRJ-023', [...path, serviceKey, 'service', 'fulfillmentScope'], `Projector does not support '${entry.service.fulfillmentScope}' fulfillment scope.`);
        }
    }
}
...
function luiEntries(core) {
    return Object.entries(core.luis);
}
