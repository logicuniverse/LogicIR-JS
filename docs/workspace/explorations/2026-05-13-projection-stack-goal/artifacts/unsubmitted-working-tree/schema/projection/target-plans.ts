...
import type { ProjectionDiagnostic, ProjectorCapabilitySet } from './types';

export type ProjectionPlanResult<TPlan> =
  | {
      ok: true;
      plan: TPlan;
...
export type PlannedEndpoint = EndpointRef;
...
export type PlannedLUI = {
  id: string;
  kind: LUKind;
  target: LUI['target'];
  ports: Record<string, PlannedPort>;
  fulfillments: string[];
  compositionSurface?: {
    outlets: string[];
...
export type PlannedConnection = {
  id: string;
  from: PlannedEndpoint;
  to: PlannedEndpoint;
  mode: 'pull' | 'push' | 'pull-push' | 'retained-current';
};

export type PlannedCore = {
  kind: LUKind;
  ports: Record<string, PlannedPort>;
  luis: Record<string, PlannedLUI>;
  connections: Record<string, PlannedConnection>;
  closures: Record<string, PlannedCore>;
  organization:
    | { kind: 'combinational' }
    | { kind: 'sequential'; steps: string[] }
    | { kind: 'stateful' }
    | {
        kind: 'structural';
        exportAnchors: Record<string, { required: boolean }>;
        externalOutlets: Record<string, CompositionAnchor>;
        exportAnchorFills: Record<string, CompositionLeaf>;
        luiFills: Record<string, Record<string, CompositionValue>>;
      };
};

export type JSRuntimePlan = {
  target: 'js-runtime';
  schemaVersion: LogicUnit['schemaVersion'];
  core: PlannedCore;
  policies: {
    async: PlannedExtension[];
    retainedCurrent: PlannedExtension[];
    dynamicFulfillment: PlannedExtension[];
    lifecycle: PlannedExtension[];
    error: PlannedExtension[];
  };
};

export type PythonRuntimePlan = {
  ...
  policies: {
    ...
    retainedCurrent: PlannedExtension[];
    dynamicFulfillment: PlannedExtension[];
    resourceLifecycle: PlannedExtension[];
    concurrency: PlannedExtension[];
    error: PlannedExtension[];
  };
};

export type HDLSignalPlan = {
  portKey: string;
...
export type HDLSliceInterfacePlan = {
  portName: string;
  width: number;
  signed: boolean;
  packed: boolean;
...
export type HDLStructuralSliceFanInPolicy = 'or' | 'and' | 'xor';

export type HDLStructuralSliceFanInPlan = {
  policy: HDLStructuralSliceFanInPolicy;
};

export type HDLStructuralSlicePlan = {
  ...
  txPort?: string;
  rxPort?: string;
  tx?: HDLSliceInterfacePlan;
  rx?: HDLSliceInterfacePlan;
  bus?: {
    routing: 'payload-path' | 'pin-channel' | 'custom';
    channelPath?: PayloadPath;
  };
  fanIn?: HDLStructuralSliceFanInPlan;
  root?: CompositionLeaf;
  childAnchorFills: Record<string, Record<string, CompositionValue>>;
  footprint: HDLStructuralSliceFootprint;
};

export type HDLStructuralSliceFootprint = {
  luis: string[];
  luiOutlets: { luiId: string; outletKey: string }[];
  externalOutlets: string[];
  anchors: { luiId: string; anchorKey: string }[];
};

export type HDLStructuralSliceLinkPlan = {
  corePath: (string | number)[];
...
export type VerilogHDLPlan = {
  target: 'verilog-hdl';
  schemaVersion: LogicUnit['schemaVersion'];
  core: PlannedCore;
  signals: HDLSignalPlan[];
  clockReset: PlannedExtension[];
  moduleBindings: PlannedExtension[];
  combinationalAssigns: PlannedExtension[];
  stateRegisters: PlannedExtension[];
  elaboration: PlannedExtension[];
  structuralSlices: PlannedExtension[];
  structuralSlicePlans: HDLStructuralSlicePlan[];
  structuralSliceLinks: HDLStructuralSliceLinkPlan[];
};

type RequirementSurfaceLike = LogicUnit['requirements'] | PlainRequirementSurface;

export function projectJSRuntimePlan(
  unit: LogicUnit,
  capabilities: ProjectorCapabilitySet
): ProjectionPlanResult<JSRuntimePlan> {
  const diagnostics = preflightForTarget(unit, capabilities, 'js-runtime');
  validateJSFailProjectionPolicy(
    collectExtensions(unit, 'logicir.js-runtime', 'error-policy'),
    unit.core,
    diagnostics
  );
  validateJSUnsupportedAsyncPolicy(
    collectExtensions(unit, 'logicir.js-runtime', 'async-policy'),
    diagnostics
  );
  validateRetainedCurrentPayloadPathSelectors(
    collectExtensions(
      unit,
      'logicir.js-runtime',
  ...
  validateJSUnsupportedDynamicFulfillmentPolicy(
    collectExtensions(unit, 'logicir.js-runtime', 'dynamic-fulfillment'),
    diagnostics
  );
  ...
  return {
    ...
    plan: {
      ...
      policies: {
        ...
        retainedCurrent: collectExtensions(
          unit,
          'logicir.js-runtime',
          'retained-current-realization'
        ),
        dynamicFulfillment: collectExtensions(
          unit,
          'logicir.js-runtime',
          'dynamic-fulfillment'
        ),
...
function validateJSFailProjectionPolicy(
  errorPolicies: PlannedExtension[],
  core: LUCore,
  diagnostics: ProjectionDiagnostic[]
): void {
  for (const policy of errorPolicies) {
    ...
    if (policy.payload.onThrow === 'fail-projection') {
      diagnostics.push({
        ...
        message:
          'Executable JS projection does not support required fail-projection error policy; fail before lowering rather than generating runtime code that cannot preserve it.',
      });
    }
...
function validateJSUnsupportedAsyncPolicy(
  asyncPolicies: PlannedExtension[],
  diagnostics: ProjectionDiagnostic[]
): void {
  for (const policy of asyncPolicies) {
    ...
    if (policy.payload.awaitBeforeNext === false) {
      diagnostics.push({
        ...
        message:
          'Executable JS projection requires awaitBeforeNext for ordered execution; fail before lowering rather than generating runtime code that cannot preserve ordering.',
      });
    }
  }
}

function validateRetainedCurrentPayloadPathSelectors(
  retainedCurrentPolicies: PlannedExtension[],
  code: string,
  targetLabel: 'JS' | 'Python',
  diagnostics: ProjectionDiagnostic[]
): void {
  for (const policy of retainedCurrentPolicies) {
    ...
    if (
      !retainedPolicyPathEndpoint(policy.path) &&
...
function validateJSUnsupportedDynamicFulfillmentPolicy(
  dynamicFulfillmentPolicies: PlannedExtension[],
  diagnostics: ProjectionDiagnostic[]
): void {
  for (const policy of dynamicFulfillmentPolicies) {
    ...
    const supported =
      (policy.payload.consistency === 'no-live-switch' &&
    ...
    if (!supported) {
      diagnostics.push({
        ...
        message:
          'Executable JS projection supports only required static-at-startup or late-bound dynamic fulfillment with no-live-switch consistency, plus switchable fulfillment at quiescent or transactional invocation boundaries; fail before lowering rather than generating runtime code that cannot preserve stronger live-switch binding.',
      });
    }
  }
}

export function projectPythonRuntimePlan(
  unit: LogicUnit,
  capabilities: ProjectorCapabilitySet
): ProjectionPlanResult<PythonRuntimePlan> {
  const diagnostics = preflightForTarget(unit, capabilities, 'python-runtime');
  validatePythonUnsupportedErrorPolicy(
    collectExtensions(unit, 'logicir.python-runtime', 'error-policy'),
    unit.core,
    diagnostics
  );
  validatePythonUnsupportedAsyncPolicy(
    collectExtensions(unit, 'logicir.python-runtime', 'async-policy'),
    diagnostics
  );
  validateRetainedCurrentPayloadPathSelectors(
    collectExtensions(
      unit,
      'logicir.python-runtime',
      'retained-current-realization'
    ),
    'PY-008',
    'Python',
  ...
  validatePythonUnsupportedDynamicFulfillmentPolicy(
    collectExtensions(unit, 'logicir.python-runtime', 'dynamic-fulfillment'),
    diagnostics
  );
  validatePythonUnsupportedResourceLifecyclePolicy(
    collectExtensions(unit, 'logicir.python-runtime', 'resource-lifecycle'),
    diagnostics
  );
  validatePythonUnsupportedConcurrencyPolicy(
    collectExtensions(unit, 'logicir.python-runtime', 'concurrency'),
    diagnostics
  );
  if (hasErrors(diagnostics)) {
    return { ok: false, diagnostics };
  ...
  return {
    ...
    plan: {
      ...
      policies: {
        ...
        resourceLifecycle: collectExtensions(
          unit,
          'logicir.python-runtime',
          'resource-lifecycle'
        ),
        concurrency: collectExtensions(
          unit,
          'logicir.python-runtime',
          'concurrency'
        ),
        error: collectExtensions(
          unit,
          'logicir.python-runtime',
          'error-policy'
        ),
...
function validatePythonUnsupportedErrorPolicy(
  errorPolicies: PlannedExtension[],
  core: LUCore,
  diagnostics: ProjectionDiagnostic[]
): void {
  for (const policy of errorPolicies) {
    ...
    if (
      policy.payload.onException === 'return-exception' ||
      ...
      diagnostics.push({
        ...
        message:
          'Executable Python projection does not support required return-exception or cancel-task error policy; fail before lowering rather than generating runtime code that cannot preserve it.',
      });
    }
...
function validateErrorPortSelector(
  policy: PlannedExtension,
  core: LUCore,
  code: string,
  targetLabel: 'JS' | 'Python',
  diagnostics: ProjectionDiagnostic[]
...
function validatePythonUnsupportedAsyncPolicy(
  asyncPolicies: PlannedExtension[],
  diagnostics: ProjectionDiagnostic[]
): void {
  for (const policy of asyncPolicies) {
    ...
    if (policy.payload.awaitBeforeNext === false) {
      diagnostics.push({
        ...
        path: [...policy.path, 'payload', 'awaitBeforeNext'],
        message:
          'Executable Python projection requires awaitBeforeNext for ordered execution; fail before lowering rather than generating runtime code that cannot preserve ordering.',
      });
    }
  }
}

function validatePythonUnsupportedDynamicFulfillmentPolicy(
  dynamicFulfillmentPolicies: PlannedExtension[],
  diagnostics: ProjectionDiagnostic[]
): void {
  for (const policy of dynamicFulfillmentPolicies) {
    ...
    const supported =
      (policy.payload.binding === 'constructor-injected' &&
  ...
  for (const policy of resourceLifecyclePolicies) {
    ...
    if (
      ...
      diagnostics.push({
        ...
        message:
          'Executable Python projection supports only required none, context-manager, async-context-manager, or start-stop resource lifecycle; fail before lowering rather than generating runtime code that cannot preserve a custom protocol.',
      });
    }
  }
}

function validatePythonUnsupportedConcurrencyPolicy(
  concurrencyPolicies: PlannedExtension[],
  diagnostics: ProjectionDiagnostic[]
): void {
  for (const policy of concurrencyPolicies) {
    ...
    if (
      ...
      diagnostics.push({
        ...
        message:
          'Executable Python projection supports only required same-thread or thread concurrency; fail before lowering rather than generating runtime code that cannot preserve task, process, or external-worker execution.',
      });
      continue;
    }
    if (execution === 'thread' && backpressure !== undefined && backpressure !== 'block') {
      diagnostics.push({
        ...
        message:
          'Executable Python projection supports only required blocking thread concurrency backpressure; fail before lowering rather than generating runtime code that cannot preserve the requested backpressure policy.',
      });
    }
    if (execution === 'thread' && ordering !== undefined && ordering !== 'preserve') {
      diagnostics.push({
        ...
        message:
          'Executable Python projection supports only required preserve ordering for thread concurrency; fail before lowering rather than generating runtime code that cannot preserve the requested ordering policy.',
      });
    }
  }
}

export function projectVerilogHDLPlan(
  unit: LogicUnit,
  capabilities: ProjectorCapabilitySet
): ProjectionPlanResult<VerilogHDLPlan> {
  const diagnostics = preflightForTarget(unit, capabilities, 'verilog-hdl');
  const structuralSlices = collectExtensions(
    unit,
    'logicir.verilog-hdl',
    'structural-slices'
  );
  ...
  validateHDLModuleBindingCoverage(
    unit.core,
    ['core'],
    collectExtensions(unit, 'logicir.verilog-hdl', 'module-binding'),
    collectExtensions(unit, 'logicir.verilog-hdl', 'combinational-assigns'),
    collectExtensions(unit, 'logicir.verilog-hdl', 'state-registers'),
    capabilities.verilogHDL?.moduleBinding === true
      ? capabilities.verilogHDL.moduleRegistry
      : undefined,
    diagnostics
  );
  validateHDLModuleRegistryInterfaces(
    unit.core,
    ['core'],
    capabilities.verilogHDL?.moduleBinding === true
      ? capabilities.verilogHDL.moduleRegistry
      : undefined,
    collectHDLSignals(unit.core, ['core']),
    diagnostics
  );
  validateHDLCombinationalAssignWidths(
    collectExtensions(unit, 'logicir.verilog-hdl', 'combinational-assigns'),
    collectHDLSignals(unit.core, ['core']),
    diagnostics
  );
  validateHDLStateRegisterWidths(
    collectExtensions(unit, 'logicir.verilog-hdl', 'state-registers'),
    collectHDLSignals(unit.core, ['core']),
    diagnostics
  );
  validateHDLStructuralSliceCoverage(
    unit.core,
    ['core'],
    structuralSlices,
    diagnostics
  );
  validateHDLStructuralSliceLinkCompatibility(
    unit.core,
    ['core'],
    structuralSlices,
    diagnostics
  ...
  const structuralSlicePlans = collectHDLStructuralSlicePlans(
    unit.core,
    ['core'],
    structuralSlices
  );
  ...
  return {
    ...
    plan: {
      ...
      clockReset: collectExtensions(
        unit,
        'logicir.verilog-hdl',
        'clock-reset'
      ),
      moduleBindings: collectExtensions(
        unit,
        'logicir.verilog-hdl',
        'module-binding'
      ).concat(collectHDLModuleRegistryBindings(unit.core, capabilities)),
      combinationalAssigns: collectExtensions(
        unit,
        'logicir.verilog-hdl',
        'combinational-assigns'
      ),
      stateRegisters: collectExtensions(
        unit,
        'logicir.verilog-hdl',
        'state-registers'
      ),
      elaboration: collectExtensions(
        unit,
        'logicir.verilog-hdl',
        'elaboration'
      ),
      structuralSlices: collectExtensions(
        unit,
        'logicir.verilog-hdl',
        'structural-slices'
      ),
...
function validateHDLCombinationalAssignWidths(
  combinationalAssigns: PlannedExtension[],
  signals: HDLSignalPlan[],
  diagnostics: ProjectionDiagnostic[]
): void {
...
function preflightForTarget(
  unit: LogicUnit,
  capabilities: ProjectorCapabilitySet,
  target: 'js-runtime' | 'python-runtime' | 'verilog-hdl'
): ProjectionDiagnostic[] {
  const diagnostics = preflightProjection(unit, capabilities);
...
function planCore(core: LUCore): PlannedCore {
  const luis: Record<string, PlannedLUI> = {};
  for (const [luiId, lui] of Object.entries(core.luis) as [string, LUI][]) {
    luis[luiId] = planLUI(luiId, lui);
  }

  const connections: Record<string, PlannedConnection> = {};
  ...
  const closures: Record<string, PlannedCore> = {};
...
function planOrganization(core: LUCore): PlannedCore['organization'] {
...
function planLUI(id: string, lui: LUI): PlannedLUI {
  return {
    id,
    kind: lui.kind,
    target: lui.target,
...
function planPorts(ports: PortSurface): Record<string, PlannedPort> {
  const planned: Record<string, PlannedPort> = {};
...
function classifyConnectionMode(
  targetPort: Port | undefined
): PlannedConnection['mode'] {
  if (!targetPort) {
    return 'pull';
  }
  ...
  if (
    targetPort.interaction.pullReadable &&
...
function validateHDLSignalCoverage(
  core: LUCore,
  path: (string | number)[],
  diagnostics: ProjectionDiagnostic[]
): void {
  validateHDLPortSignalCoverage(
    core.ports,
    core.extensions,
    [...path, 'ports'],
    diagnostics
...
function validateHDLStateRegisterWidths(
  stateRegisters: PlannedExtension[],
  signals: HDLSignalPlan[],
  diagnostics: ProjectionDiagnostic[]
): void {
...
function validateHDLStructuralSliceCoverage(
  core: LUCore,
  path: (string | number)[],
  structuralSlices: PlannedExtension[],
  diagnostics: ProjectionDiagnostic[]
...
function validateHDLStructuralSliceLinkCompatibility(
  core: LUCore,
  path: (string | number)[],
  structuralSlices: PlannedExtension[],
  diagnostics: ProjectionDiagnostic[]
  ...
  if (core.kindOrganization.kind === 'structural') {
    ...
    if (sliceExtension?.requirement === 'required') {
      ...
      const providersByOutlet = new Map<string, HDLStructuralSlicePlan[]>();
      ...
      for (const consumer of plans) {
        ...
        for (const outlet of consumer.footprint.luiOutlets) {
          const providers =
            providersByOutlet.get(`${outlet.luiId}:${outlet.outletKey}`) ?? [];
          for (const provider of providers) {
            ...
            if (sameSliceInterface(provider.tx, consumer.rx)) {
              compatibleProviders.push(
                `${provider.anchorKey}:${outlet.luiId}.${outlet.outletKey}`
              );
              continue;
            }
            diagnostics.push({
...
function validateHDLClockResetCoverage(
  core: LUCore,
  path: (string | number)[],
  diagnostics: ProjectionDiagnostic[]
): void {
  ...
  if (
    ...
    diagnostics.push({
      ...
      message:
        'Verilog HDL projection requires a clock-reset extension for sequential or stateful cores.',
    });
  }

  for (const [luiId, lui] of Object.entries(core.luis) as [string, LUI][]) {
    if (
      (lui.kind === 'sequential' || lui.kind === 'stateful') &&
      !coreClockReset &&
      ...
      diagnostics.push({
        ...
        message:
          'Verilog HDL projection requires a clock-reset extension for sequential or stateful LUI instances.',
      });
    }
  }

  for (const [closureId, closure] of Object.entries(core.closures)) {
    validateHDLClockResetCoverage(
      closure.core,
      [...path, 'closures', closureId, 'core'],
      diagnostics
    );
...
function collectHDLStructuralSlicePlans(
  core: LUCore,
  path: (string | number)[],
  structuralSlices: PlannedExtension[]
): HDLStructuralSlicePlan[] {
  const plans: HDLStructuralSlicePlan[] = [];
  if (core.kindOrganization.kind === 'structural') {
    ...
    for (const [anchorKey, anchor] of Object.entries(
      ...
      plans.push({
        ...
        moduleName:
          typeof slice.moduleName === 'string'
            ? slice.moduleName
            : defaultSliceModuleName(path, anchorKey),
        placement:
          typeof slice.placement === 'string' ? slice.placement : undefined,
        ...
        footprint: collectStructuralSliceFootprint(
          core.kindOrganization.exportAnchorFills[anchorKey],
          core.kindOrganization.luiFills
        ),
      });
    }
  }
  ...
  for (const [closureId, closure] of Object.entries(core.closures)) {
    plans.push(
      ...collectHDLStructuralSlicePlans(
        closure.core,
        [...path, 'closures', closureId, 'core'],
        structuralSlices
      )
...
function collectStructuralSliceFootprint(
  root: CompositionLeaf | undefined,
  childAnchorFills: Record<string, Record<string, CompositionValue>>
): HDLStructuralSliceFootprint {
  const luis = new Set<string>();
  const luiOutlets = new Map<string, { luiId: string; outletKey: string }>();
  const externalOutlets = new Set<string>();
  const anchors = new Map<string, { luiId: string; anchorKey: string }>();
  const visitedFills = new Set<string>();

  const visitLeaf = (leaf: CompositionLeaf | undefined): void => {
  ...
  const visitValue = (value: CompositionValue): void => {
...
function collectHDLStructuralSliceLinks(
  slices: HDLStructuralSlicePlan[]
): HDLStructuralSliceLinkPlan[] {
  const links: HDLStructuralSliceLinkPlan[] = [];
  const providersByOutlet = new Map<string, HDLStructuralSlicePlan[]>();
  ...
  for (const consumer of slices) {
    ...
    const candidates: HDLStructuralSliceLinkPlan[] = [];
    for (const outlet of consumer.footprint.luiOutlets) {
      const providers =
        providersByOutlet.get(`${outlet.luiId}:${outlet.outletKey}`) ?? [];
      for (const provider of providers) {
  ...
  return links.sort(
    (left, right) =>
      left.fromAnchorKey.localeCompare(right.fromAnchorKey) ||
...
function sameSliceInterface(
  left: HDLSliceInterfacePlan,
  right: HDLSliceInterfacePlan
): boolean {
  return (
...
function compareLUIOutletRefs(
  left: { luiId: string; outletKey: string },
  right: { luiId: string; outletKey: string }
): number {
  return (
    left.luiId.localeCompare(right.luiId) ||
    left.outletKey.localeCompare(right.outletKey)
...
function compareLUIAnchorRefs(
  left: { luiId: string; anchorKey: string },
  right: { luiId: string; anchorKey: string }
): number {
  return (
    left.luiId.localeCompare(right.luiId) ||
...
function findStructuralSlicesForCore(
  structuralSlices: PlannedExtension[],
  corePath: (string | number)[]
): PlannedExtension | undefined {
  return structuralSlices.find((extension) =>
    extension.path.length >= corePath.length + 2 &&
    corePath.every((segment, index) => extension.path[index] === segment) &&
    extension.path[corePath.length] === 'extensions'
  );
}

function defaultSliceModuleName(
  corePath: (string | number)[],
  anchorKey: string
): string {
...
function collectHDLSliceInterface(
  value: unknown,
  fallbackPortName: unknown,
  defaultPortName: string
): HDLSliceInterfacePlan | undefined {
...
function collectHDLSliceFanIn(
  value: unknown
): HDLStructuralSliceFanInPlan | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function validateHDLModuleBindingCoverage(
  core: LUCore,
  path: (string | number)[],
  moduleBindings: PlannedExtension[],
  combinationalAssigns: PlannedExtension[],
  stateRegisters: PlannedExtension[],
  moduleRegistry: NonNullable<ProjectorCapabilitySet['verilogHDL']>['moduleRegistry'] | undefined,
  diagnostics: ProjectionDiagnostic[]
): void {
  for (const [luiId, lui] of Object.entries(core.luis) as [string, LUI][]) {
    ...
    if (
      ...
      diagnostics.push({
        ...
        path: [...path, 'luis', luiId, 'extensions'],
        message:
          'Verilog HDL projection requires module-binding or explicit combinational behavior for external LUI targets.',
      });
    }
  }
...
function collectHDLModuleRegistryBindings(
  core: LUCore,
  capabilities: ProjectorCapabilitySet
): PlannedExtension[] {
  ...
  const bindings: PlannedExtension[] = [];
  collectCoreHDLModuleRegistryBindings(core, ['core'], moduleRegistry, bindings);
  return bindings;
}

function collectCoreHDLModuleRegistryBindings(
  core: LUCore,
  path: (string | number)[],
  moduleRegistry: NonNullable<ProjectorCapabilitySet['verilogHDL']>['moduleRegistry'],
  bindings: PlannedExtension[]
): void {
  for (const [luiId, lui] of Object.entries(core.luis) as [string, LUI][]) {
    if (
      lui.target.kind !== 'external' ||
  ...
  for (const [closureId, closure] of Object.entries(core.closures)) {
    collectCoreHDLModuleRegistryBindings(
      closure.core,
      [...path, 'closures', closureId, 'core'],
      moduleRegistry,
      bindings
...
function findHDLModuleRegistryBinding(
  moduleRegistry: NonNullable<ProjectorCapabilitySet['verilogHDL']>['moduleRegistry'] | undefined,
  target: Extract<LUI['target'], { kind: 'external' }>
): NonNullable<
...
function findHDLModuleBindingInExtensions(
  extensions: ExtensionRecord[] | undefined
): ExtensionRecord | undefined {
  return extensions?.find((extension) =>
    matchesExtension(extension, 'logicir.verilog-hdl', 'module-binding')
  );
}

function validateHDLModuleRegistryInterfaces(
  core: LUCore,
  path: (string | number)[],
  moduleRegistry: NonNullable<ProjectorCapabilitySet['verilogHDL']>['moduleRegistry'] | undefined,
  signals: HDLSignalPlan[],
...
function validateCoreHDLModuleRegistryInterfaces(
  core: LUCore,
  path: (string | number)[],
  moduleRegistry: NonNullable<ProjectorCapabilitySet['verilogHDL']>['moduleRegistry'],
  ...
  for (const [luiId, lui] of Object.entries(core.luis) as [string, LUI][]) {
    ...
    for (const [portKey, port] of Object.entries(lui.ports)) {
      const externalName =
        typeof portMap[portKey] === 'string' ? portMap[portKey] : portKey;
      ...
      if (
        typeof contract.direction === 'string' &&
      ...
      const signal = signalMap.get(
        endpointSignalKey({ owner: { kind: 'lui', luiId }, portKey })
      );
      if (
        signal &&
  ...
  for (const [closureId, closure] of Object.entries(core.closures)) {
    validateCoreHDLModuleRegistryInterfaces(
      closure.core,
      [...path, 'closures', closureId, 'core'],
      moduleRegistry,
      signalMap,
...
function validateHDLPortSignalCoverage(
  ports: PortSurface,
  ownerExtensions: ExtensionRecord[] | undefined,
  path: (string | number)[],
  diagnostics: ProjectionDiagnostic[]
): void {
  for (const [portKey, port] of Object.entries(ports)) {
...
function collectHDLSignals(
  core: LUCore,
  path: (string | number)[]
): HDLSignalPlan[] {
  const signals: HDLSignalPlan[] = [];
  collectHDLPortSignals(
    core.ports,
    core.extensions,
    [...path, 'ports'],
    signals
  ...
  for (const [closureId, closure] of Object.entries(core.closures)) {
    signals.push(
      ...collectHDLSignals(closure.core, [
        ...path,
        'closures',
        closureId,
        'core',
...
function collectHDLPortSignals(
  ports: PortSurface,
  ownerExtensions: ExtensionRecord[] | undefined,
  path: (string | number)[],
  signals: HDLSignalPlan[]
  ...
  for (const [portKey, port] of Object.entries(ports)) {
    for (const signal of collectPortHDLSignalTypes(
      portKey,
      port,
      ownerExtensions
    )) {
      const candidate: HDLSignalPlan = {
        portKey,
        payloadPath:
          signal.payloadPath.length > 0 ? signal.payloadPath : undefined,
...
function findWholePortHDLSignalType(
  portKey: string,
  portExtensions: ExtensionRecord[] | undefined,
  ownerExtensions: ExtensionRecord[] | undefined
): CollectedHDLSignalType | undefined {
  return collectPortHDLSignalTypes(
    portKey,
    { extensions: portExtensions },
    ownerExtensions
  ).find((signal) => signal.payloadPath.length === 0);
}

function collectPortHDLSignalTypes(
  portKey: string,
  port: { extensions?: ExtensionRecord[] },
  ownerExtensions: ExtensionRecord[] | undefined
): CollectedHDLSignalType[] {
  return [
...
function collectHDLSignalTypesFromExtensions(
  extensions: ExtensionRecord[] | undefined,
  portKey: string,
  allowImplicitPort: boolean
): CollectedHDLSignalType[] {
  const signals: CollectedHDLSignalType[] = [];
...
function selectedHDLPayloadPath(selector: unknown): PayloadPath {
  if (!isRecord(selector)) {
    return [];
  }
...
function retainedPolicyPathEndpoint(
  path: (string | number)[]
): { owner: 'lu' | 'lui'; portKey: string } | undefined {
...
function hasCombinationalRealizationForLUI(
  combinationalAssigns: PlannedExtension[],
  luiId: string
): boolean {
...
function hasStateRegisterRealizationForLUI(
  stateRegisters: PlannedExtension[],
  luiId: string
): boolean {
  return stateRegisters.some((extension) => {
    ...
    return payload.registers.some((register) => {
      ...
      return (
        register.target.owner.kind === 'lui' &&
...
function walkExtensions(
  unit: LogicUnit,
  visit: (extension: ExtensionRecord, path: (string | number)[]) => void
): void {
  ...
  for (const [luiId, lui] of Object.entries(core.luis) as [string, LUI][]) {
    ...
    if (lui.kind === 'structural') {
      visitExtensionList(
        ...
        [...luiPath, 'compositionSurface', 'extensions'],
        visit
      );
    }
...
function walkRequirementSurfaceExtensions(
  ...
  for (const [serviceKey, entry] of Object.entries(surface)) {
    ...
    for (const [unitKey, unit] of Object.entries(entry.service.units)) {
      ...
      if (hasNestedRequirements(unit)) {
        walkRequirementSurfaceExtensions(
          unit.requirements,
      ...
      if (unit.kind === 'structural') {
        visitExtensionList(
          unit.compositionSurface.extensions,
...
function matchesExtension(
  ...
  return (
    extension.feature.namespace === namespace &&
    (extension.key === key || extension.feature.key === key)
  );
}

function hasClockResetExtension(
  extensions: ExtensionRecord[] | undefined
): boolean {
  return (
    extensions?.some((extension) =>
      matchesExtension(extension, 'logicir.verilog-hdl', 'clock-reset')
    ) ?? false
  );
}

function hasErrors(diagnostics: ProjectionDiagnostic[]): boolean {
...
function isHDLStructuralSliceRouting(
  value: unknown
): value is 'payload-path' | 'pin-channel' | 'custom' {
  return value === 'payload-path' || value === 'pin-channel' || value === 'custom';
}

function isHDLStructuralSliceFanInPolicy(
  value: unknown
): value is HDLStructuralSliceFanInPolicy {
  return value === 'or' || value === 'and' || value === 'xor';
}

function isHDLEncoding(
  value: unknown
): value is 'bits' | 'one-hot' | 'gray' | 'custom' {
  return (
    value === 'bits' ||
    value === 'one-hot' ||
    value === 'gray' ||
...
function isPayloadPath(value: unknown): value is PayloadPath {
  return (
    Array.isArray(value) &&
    value.every(
      (segment) =>
...
function inferHDLExpressionWidth(
  expr: unknown,
  signalMap: Map<string, HDLSignalPlan>
): number | undefined {
  if (!isRecord(expr)) {
...
function hasNestedRequirements(
  value: unknown
): value is { requirements: RequirementSurfaceLike } {
