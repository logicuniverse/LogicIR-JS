...
const result = (0, target_plans_1.projectPythonRuntimePlan)(unit, capabilities);
assert(result.ok, 'Python runtime plan should be valid for typed adder.');
if (!result.ok) {
    throw new Error('unreachable');
}
const artifact = (0, lowering_1.lowerPythonExecutableRuntime)(result.plan);
...
try {
    ...
    assert(completed.status === 0, `Python smoke failed with exit ${String(completed.status)}\n${completed.stdout}\n${completed.stderr}`);
    const pathUnit = withPayloadPathBusTypes(payloadPathAdderUnit());
    const pathResult = (0, target_plans_1.projectPythonRuntimePlan)(pathUnit, capabilities);
    assert(pathResult.ok, 'Python runtime plan should support typed payloadPath unit.');
    if (!pathResult.ok) {
        throw new Error('unreachable');
    }
    const pathArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(pathResult.plan);
    ...
    assert(pathCompleted.status === 0, `Python payloadPath smoke failed with exit ${String(pathCompleted.status)}\n${pathCompleted.stdout}\n${pathCompleted.stderr}`);
    const pinUnit = withPinBusTypes(pinAwareAdderUnit());
    const pinResult = (0, target_plans_1.projectPythonRuntimePlan)(pinUnit, capabilities);
    assert(pinResult.ok, 'Python runtime plan should support pin-aware unit.');
    if (!pinResult.ok) {
        throw new Error('unreachable');
    }
    const pinArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(pinResult.plan);
    ...
    assert(pinCompleted.status === 0, `Python pin-aware smoke failed with exit ${String(pinCompleted.status)}\n${pinCompleted.stdout}\n${pinCompleted.stderr}`);
    const sequentialUnit = withSequentialPayloadTypes(examples_1.sequentialPipelineExample);
    const sequentialResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonAsyncPolicy(sequentialUnit, {
        invocation: 'sync-call',
        awaitBeforeNext: true,
        selector: { stepIndex: 1 },
    }), capabilities);
    assert(sequentialResult.ok, 'Python runtime plan should support typed sequential pipeline.');
    if (!sequentialResult.ok) {
        throw new Error('unreachable');
    }
    const sequentialArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(sequentialResult.plan);
    const sequentialFile = join(dir, `sequential_${sequentialArtifact.filename}`);
    writeFileSync(sequentialFile, sequentialArtifact.content, 'utf8');
    const sequentialRunner = join(dir, 'run_sequential_smoke.py');
    writeFileSync(sequentialRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    assert(sequentialCompleted.status === 0, `Python sequential smoke failed with exit ${String(sequentialCompleted.status)}\n${sequentialCompleted.stdout}\n${sequentialCompleted.stderr}`);
    const coroutineResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonAsyncPolicy(sequentialUnit, {
        invocation: 'coroutine',
        awaitBeforeNext: true,
    }), capabilities);
    assert(coroutineResult.ok, 'Python runtime plan should support coroutine when capability declares it.');
    if (!coroutineResult.ok) {
        throw new Error('unreachable');
    }
    const coroutineArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(coroutineResult.plan);
    const coroutineFile = join(dir, `coroutine_${coroutineArtifact.filename}`);
    writeFileSync(coroutineFile, coroutineArtifact.content, 'utf8');
    const coroutineRunner = join(dir, 'run_coroutine_smoke.py');
    writeFileSync(coroutineRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    assert(coroutineCompleted.status === 0, `Python coroutine smoke failed with exit ${String(coroutineCompleted.status)}\n${coroutineCompleted.stdout}\n${coroutineCompleted.stderr}`);
    const generatorResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonAsyncPolicy(sequentialUnit, {
        invocation: 'generator',
        awaitBeforeNext: true,
    }), capabilities);
    assert(generatorResult.ok, 'Python runtime plan should support generator when capability declares it.');
    if (!generatorResult.ok) {
        throw new Error('unreachable');
    }
    const generatorArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(generatorResult.plan);
    const generatorFile = join(dir, `generator_${generatorArtifact.filename}`);
    writeFileSync(generatorFile, generatorArtifact.content, 'utf8');
    const generatorRunner = join(dir, 'run_generator_smoke.py');
    writeFileSync(generatorRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    assert(generatorCompleted.status === 0, `Python generator smoke failed with exit ${String(generatorCompleted.status)}\n${generatorCompleted.stdout}\n${generatorCompleted.stderr}`);
    const asyncGeneratorResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonAsyncPolicy(sequentialUnit, {
        invocation: 'async-generator',
        awaitBeforeNext: true,
    }), capabilities);
    assert(asyncGeneratorResult.ok, 'Python runtime plan should support async-generator when capability declares it.');
    if (!asyncGeneratorResult.ok) {
        throw new Error('unreachable');
    }
    const asyncGeneratorArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(asyncGeneratorResult.plan);
    const asyncGeneratorFile = join(dir, `async_generator_${asyncGeneratorArtifact.filename}`);
    ...
    writeFileSync(asyncGeneratorRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    assert(asyncGeneratorCompleted.status === 0, `Python async-generator smoke failed with exit ${String(asyncGeneratorCompleted.status)}\n${asyncGeneratorCompleted.stdout}\n${asyncGeneratorCompleted.stderr}`);
    const threadpoolResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonAsyncPolicy(sequentialUnit, {
        invocation: 'threadpool-call',
        awaitBeforeNext: true,
    }), capabilities);
    assert(threadpoolResult.ok, 'Python runtime plan should support threadpool-call when capability declares it.');
    if (!threadpoolResult.ok) {
        throw new Error('unreachable');
    }
    const threadpoolArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(threadpoolResult.plan);
    const threadpoolFile = join(dir, `threadpool_${threadpoolArtifact.filename}`);
    writeFileSync(threadpoolFile, threadpoolArtifact.content, 'utf8');
    const threadpoolRunner = join(dir, 'run_threadpool_smoke.py');
    writeFileSync(threadpoolRunner, [
        'import importlib.util',
        'import pathlib',
        'import threading',
        '',
    ...
    assert(threadpoolCompleted.status === 0, `Python threadpool smoke failed with exit ${String(threadpoolCompleted.status)}\n${threadpoolCompleted.stdout}\n${threadpoolCompleted.stderr}`);
    const unsupportedOrderingResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonAsyncPolicy(sequentialUnit, {
        invocation: 'sync-call',
        awaitBeforeNext: false,
    }), capabilities);
    assert(!unsupportedOrderingResult.ok &&
        unsupportedOrderingResult.diagnostics.some((diagnostic) => diagnostic.code === 'PY-004'), 'Python runtime planner should reject awaitBeforeNext=false before lowering.');
    const staticDynamicResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonDynamicFulfillmentPolicy(unit, {
        selector: { serviceKey: 'auth', unitKey: 'check' },
        binding: 'constructor-injected',
        consistency: 'startup-only',
    }), capabilities);
    assert(staticDynamicResult.ok, 'Python runtime plan should support constructor-injected dynamic fulfillment.');
    if (!staticDynamicResult.ok) {
        throw new Error('unreachable');
    }
    const staticDynamicArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(staticDynamicResult.plan);
    const staticDynamicFile = join(dir, `static_dynamic_${staticDynamicArtifact.filename}`);
    ...
    writeFileSync(staticDynamicRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    assert(staticDynamicCompleted.status === 0, `Python static dynamic smoke failed with exit ${String(staticDynamicCompleted.status)}\n${staticDynamicCompleted.stdout}\n${staticDynamicCompleted.stderr}`);
    const requirementDynamicResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonDynamicFulfillmentPolicy(withAuthRequirementPayloadTypes(examples_1.childWithRequirementContractExample), {
        selector: { serviceKey: 'auth', unitKey: 'check' },
        binding: 'constructor-injected',
        consistency: 'startup-only',
    }), capabilities);
    assert(requirementDynamicResult.ok, 'Python runtime plan should support requirement-target LUI dynamic fulfillment.');
    if (!requirementDynamicResult.ok) {
        throw new Error('unreachable');
    }
    const requirementDynamicArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(requirementDynamicResult.plan);
    const requirementDynamicFile = join(dir, `requirement_dynamic_${requirementDynamicArtifact.filename}`);
    writeFileSync(requirementDynamicFile, requirementDynamicArtifact.content, 'utf8');
    const requirementDynamicRunner = join(dir, 'run_requirement_dynamic_smoke.py');
    writeFileSync(requirementDynamicRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const requirementDynamicCompleted = spawnSync('python', [requirementDynamicRunner], { encoding: 'utf8' });
    if (requirementDynamicCompleted.error) {
        throw requirementDynamicCompleted.error;
    }
    assert(requirementDynamicCompleted.status === 0, `Python requirement dynamic smoke failed with exit ${String(requirementDynamicCompleted.status)}\n${requirementDynamicCompleted.stdout}\n${requirementDynamicCompleted.stderr}`);
    const lateBoundRequirementResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonDynamicFulfillmentPolicy(withAuthRequirementPayloadTypes(examples_1.childWithRequirementContractExample), {
        selector: { serviceKey: 'auth', unitKey: 'check' },
        binding: 'late-bound',
        consistency: 'task-local',
    }), capabilities);
    assert(lateBoundRequirementResult.ok, 'Python runtime plan should support late-bound requirement-target fulfillment.');
    if (!lateBoundRequirementResult.ok) {
        throw new Error('unreachable');
    }
    const lateBoundRequirementArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(lateBoundRequirementResult.plan);
    const lateBoundRequirementFile = join(dir, `late_bound_requirement_${lateBoundRequirementArtifact.filename}`);
    writeFileSync(lateBoundRequirementFile, lateBoundRequirementArtifact.content, 'utf8');
    const lateBoundRequirementRunner = join(dir, 'run_late_bound_requirement_smoke.py');
    writeFileSync(lateBoundRequirementRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const lateBoundRequirementCompleted = spawnSync('python', [lateBoundRequirementRunner], { encoding: 'utf8' });
    if (lateBoundRequirementCompleted.error) {
        throw lateBoundRequirementCompleted.error;
    }
    assert(lateBoundRequirementCompleted.status === 0, `Python late-bound requirement smoke failed with exit ${String(lateBoundRequirementCompleted.status)}\n${lateBoundRequirementCompleted.stdout}\n${lateBoundRequirementCompleted.stderr}`);
    const switchableRequirementResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonDynamicFulfillmentPolicy(withAuthRequirementPayloadTypes(examples_1.childWithRequirementContractExample), {
        selector: { serviceKey: 'auth', unitKey: 'check' },
        binding: 'switchable',
        consistency: 'quiescent-switch',
    }), capabilities);
    assert(switchableRequirementResult.ok, 'Python runtime plan should support switchable fulfillment at quiescent invocation boundaries.');
    if (!switchableRequirementResult.ok) {
        throw new Error('unreachable');
    }
    const switchableRequirementArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(switchableRequirementResult.plan);
    const switchableRequirementFile = join(dir, `switchable_requirement_${switchableRequirementArtifact.filename}`);
    writeFileSync(switchableRequirementFile, switchableRequirementArtifact.content, 'utf8');
    const switchableRequirementRunner = join(dir, 'run_switchable_requirement_smoke.py');
    writeFileSync(switchableRequirementRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const switchableRequirementCompleted = spawnSync('python', [switchableRequirementRunner], { encoding: 'utf8' });
    if (switchableRequirementCompleted.error) {
        throw switchableRequirementCompleted.error;
    }
    assert(switchableRequirementCompleted.status === 0, `Python switchable requirement smoke failed with exit ${String(switchableRequirementCompleted.status)}\n${switchableRequirementCompleted.stdout}\n${switchableRequirementCompleted.stderr}`);
    const transactionalRequirementResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonDynamicFulfillmentPolicy(withDuplicatedAuthCheckLUI(withAuthRequirementPayloadTypes(examples_1.childWithRequirementContractExample)), {
        selector: { serviceKey: 'auth', unitKey: 'check' },
        binding: 'switchable',
        consistency: 'transactional-switch',
    }), capabilities);
    assert(transactionalRequirementResult.ok, 'Python runtime plan should support transactional switchable fulfillment with per-invocation supplier snapshots.');
    if (!transactionalRequirementResult.ok) {
        throw new Error('unreachable');
    }
    const transactionalRequirementArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(transactionalRequirementResult.plan);
    const transactionalRequirementFile = join(dir, `transactional_requirement_${transactionalRequirementArtifact.filename}`);
    writeFileSync(transactionalRequirementFile, transactionalRequirementArtifact.content, 'utf8');
    const transactionalRequirementRunner = join(dir, 'run_transactional_requirement_smoke.py');
    writeFileSync(transactionalRequirementRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const transactionalRequirementCompleted = spawnSync('python', [transactionalRequirementRunner], { encoding: 'utf8' });
    if (transactionalRequirementCompleted.error) {
        throw transactionalRequirementCompleted.error;
    }
    assert(transactionalRequirementCompleted.status === 0, `Python transactional requirement smoke failed with exit ${String(transactionalRequirementCompleted.status)}\n${transactionalRequirementCompleted.stdout}\n${transactionalRequirementCompleted.stderr}`);
    const retainedPropertyUnit = withPythonRetainedCurrentPolicy(examples_1.statefulRetainedCounterExample, {
        realization: 'source-property',
        notification: 'callback',
    });
    const retainedPropertyResult = (0, target_plans_1.projectPythonRuntimePlan)(withStatefulCounterPayloadTypes(retainedPropertyUnit), capabilities);
    assert(retainedPropertyResult.ok, 'Python runtime plan should support source-property retained-current.');
    if (!retainedPropertyResult.ok) {
        throw new Error('unreachable');
    }
    const retainedPropertyArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(retainedPropertyResult.plan);
    const retainedPropertyFile = join(dir, `retained_property_${retainedPropertyArtifact.filename}`);
    ...
    writeFileSync(retainedPropertyRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const retainedPropertyCompleted = spawnSync('python', [retainedPropertyRunner], { encoding: 'utf8' });
    if (retainedPropertyCompleted.error) {
        throw retainedPropertyCompleted.error;
    }
    assert(retainedPropertyCompleted.status === 0, `Python retained-current source-property smoke failed with exit ${String(retainedPropertyCompleted.status)}\n${retainedPropertyCompleted.stdout}\n${retainedPropertyCompleted.stderr}`);
    const retainedQueueResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonRetainedCurrentPolicy(withStatefulCounterPayloadTypes(examples_1.statefulRetainedCounterExample), {
        realization: 'asyncio-queue-latest',
        notification: 'queue',
    }), capabilities);
    assert(retainedQueueResult.ok, 'Python runtime plan should support asyncio-queue-latest retained-current realization.');
    if (!retainedQueueResult.ok) {
        throw new Error('unreachable');
    }
    const retainedQueueArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(retainedQueueResult.plan);
    const retainedQueueFile = join(dir, `retained_queue_${retainedQueueArtifact.filename}`);
    ...
    writeFileSync(retainedQueueRunner, [
        'import asyncio',
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const retainedQueueCompleted = spawnSync('python', [retainedQueueRunner], { encoding: 'utf8' });
    if (retainedQueueCompleted.error) {
        throw retainedQueueCompleted.error;
    }
    assert(retainedQueueCompleted.status === 0, `Python retained-current asyncio-queue-latest smoke failed with exit ${String(retainedQueueCompleted.status)}\n${retainedQueueCompleted.stdout}\n${retainedQueueCompleted.stderr}`);
    const retainedObservableResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonRetainedCurrentPolicy(withStatefulCounterPayloadTypes(examples_1.statefulRetainedCounterExample), {
        realization: 'observable',
        notification: 'callback',
    }), capabilities);
    assert(retainedObservableResult.ok, 'Python runtime plan should support observable callback retained-current realization.');
    if (!retainedObservableResult.ok) {
        throw new Error('unreachable');
    }
    const retainedObservableArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(retainedObservableResult.plan);
    const retainedObservableFile = join(dir, `retained_observable_${retainedObservableArtifact.filename}`);
    writeFileSync(retainedObservableFile, retainedObservableArtifact.content, 'utf8');
    const retainedObservableRunner = join(dir, 'run_retained_observable_smoke.py');
    writeFileSync(retainedObservableRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const retainedObservableCompleted = spawnSync('python', [retainedObservableRunner], { encoding: 'utf8' });
    if (retainedObservableCompleted.error) {
        throw retainedObservableCompleted.error;
    }
    assert(retainedObservableCompleted.status === 0, `Python retained-current observable callback smoke failed with exit ${String(retainedObservableCompleted.status)}\n${retainedObservableCompleted.stdout}\n${retainedObservableCompleted.stderr}`);
    const retainedPathObservableResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonLUIRetainedCurrentPolicy(withStatefulCounterVectorPayloadTypes(examples_1.statefulRetainedCounterExample), 'counterState', 'value', {
        selector: { portKey: 'value', payloadPath: ['lanes', 0] },
        realization: 'observable',
        notification: 'callback',
    }), capabilities);
    assert(retainedPathObservableResult.ok, 'Python runtime plan should support payloadPath-scoped observable retained-current realization.');
    if (!retainedPathObservableResult.ok) {
        throw new Error('unreachable');
    }
    const retainedPathObservableArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(retainedPathObservableResult.plan);
    const retainedPathObservableFile = join(dir, `retained_path_observable_${retainedPathObservableArtifact.filename}`);
    writeFileSync(retainedPathObservableFile, retainedPathObservableArtifact.content, 'utf8');
    const retainedPathObservableRunner = join(dir, 'run_retained_path_observable_smoke.py');
    writeFileSync(retainedPathObservableRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const retainedPathObservableCompleted = spawnSync('python', [retainedPathObservableRunner], { encoding: 'utf8' });
    if (retainedPathObservableCompleted.error) {
        throw retainedPathObservableCompleted.error;
    }
    assert(retainedPathObservableCompleted.status === 0, `Python payloadPath retained-current observable smoke failed with exit ${String(retainedPathObservableCompleted.status)}\n${retainedPathObservableCompleted.stdout}\n${retainedPathObservableCompleted.stderr}`);
    const explicitOwnerRetainedPathResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonRetainedCurrentPolicy(withStatefulCounterVectorPayloadTypes(examples_1.statefulRetainedCounterExample), {
        selector: {
            luiId: 'counterState',
            portKey: 'value',
            payloadPath: ['lanes', 0],
    ...
    assert(explicitOwnerRetainedPathResult.ok, 'Python runtime plan should support core-level explicit LUI retained-current payloadPath selector.');
    if (!explicitOwnerRetainedPathResult.ok) {
        throw new Error('unreachable');
    }
    const explicitOwnerRetainedPathArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(explicitOwnerRetainedPathResult.plan);
    const explicitOwnerRetainedPathFile = join(dir, `explicit_owner_retained_path_${explicitOwnerRetainedPathArtifact.filename}`);
    writeFileSync(explicitOwnerRetainedPathFile, explicitOwnerRetainedPathArtifact.content, 'utf8');
    const explicitOwnerRetainedPathRunner = join(dir, 'run_explicit_owner_retained_path_smoke.py');
    writeFileSync(explicitOwnerRetainedPathRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const explicitOwnerRetainedPathCompleted = spawnSync('python', [explicitOwnerRetainedPathRunner], { encoding: 'utf8' });
    if (explicitOwnerRetainedPathCompleted.error) {
        throw explicitOwnerRetainedPathCompleted.error;
    }
    assert(explicitOwnerRetainedPathCompleted.status === 0, `Python explicit-owner payloadPath retained-current observable smoke failed with exit ${String(explicitOwnerRetainedPathCompleted.status)}\n${explicitOwnerRetainedPathCompleted.stdout}\n${explicitOwnerRetainedPathCompleted.stderr}`);
    const retainedPollResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonRetainedCurrentPolicy(withStatefulCounterPayloadTypes(examples_1.statefulRetainedCounterExample), {
        realization: 'observable',
        notification: 'poll',
    }), capabilities);
    assert(retainedPollResult.ok, 'Python runtime plan should support observable poll retained-current realization.');
    if (!retainedPollResult.ok) {
        throw new Error('unreachable');
    }
    const retainedPollArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(retainedPollResult.plan);
    const retainedPollFile = join(dir, `retained_poll_${retainedPollArtifact.filename}`);
    ...
    writeFileSync(retainedPollRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const retainedPollCompleted = spawnSync('python', [retainedPollRunner], { encoding: 'utf8' });
    if (retainedPollCompleted.error) {
        throw retainedPollCompleted.error;
    }
    assert(retainedPollCompleted.status === 0, `Python retained-current observable poll smoke failed with exit ${String(retainedPollCompleted.status)}\n${retainedPollCompleted.stdout}\n${retainedPollCompleted.stderr}`);
    const adapterRetainedUnit = withPythonRetainedCurrentPolicy(withStatefulCounterPayloadTypes(examples_1.statefulRetainedCounterExample), {
        realization: 'projector-adapter',
        notification: 'poll',
    });
    const adapterRetainedResult = (0, target_plans_1.projectPythonRuntimePlan)(adapterRetainedUnit, capabilities);
    assert(adapterRetainedResult.ok, 'Python runtime plan should support projector-adapter retained-current.');
    if (!adapterRetainedResult.ok) {
        throw new Error('unreachable');
    }
    const adapterRetainedArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(adapterRetainedResult.plan);
    const adapterRetainedFile = join(dir, `adapter_retained_${adapterRetainedArtifact.filename}`);
    ...
    writeFileSync(adapterRetainedRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const adapterRetainedCompleted = spawnSync('python', [adapterRetainedRunner], { encoding: 'utf8' });
    if (adapterRetainedCompleted.error) {
        throw adapterRetainedCompleted.error;
    }
    assert(adapterRetainedCompleted.status === 0, `Python retained-current projector-adapter smoke failed with exit ${String(adapterRetainedCompleted.status)}\n${adapterRetainedCompleted.stdout}\n${adapterRetainedCompleted.stderr}`);
    const ownerlessPathRetainedResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonRetainedCurrentPolicy(withStatefulCounterVectorPayloadTypes(examples_1.statefulRetainedCounterExample), {
        selector: { portKey: 'value', payloadPath: ['lanes', 0] },
        realization: 'observable',
        notification: 'callback',
    }), capabilities);
    assert(!ownerlessPathRetainedResult.ok &&
        ownerlessPathRetainedResult.diagnostics.some((diagnostic) => diagnostic.code === 'PY-008'), 'Python runtime planner should reject payloadPath retained-current policies without a concrete port attachment.');
    const raiseErrorResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonErrorPolicy(unit, {
        selector: { luiId: 'adder' },
        onException: 'raise',
    }), capabilities);
    assert(raiseErrorResult.ok, 'Python runtime plan should support required raise error policy.');
    if (!raiseErrorResult.ok) {
        throw new Error('unreachable');
    }
    const raiseErrorArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(raiseErrorResult.plan);
    ...
    writeFileSync(raiseErrorRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    assert(raiseErrorCompleted.status === 0, `Python raise error smoke failed with exit ${String(raiseErrorCompleted.status)}\n${raiseErrorCompleted.stdout}\n${raiseErrorCompleted.stderr}`);
    const useErrorPortResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonErrorPolicy(withPythonErrorPort(unit), {
        selector: { luiId: 'adder', portKey: 'error' },
        onException: 'use-error-port',
    }), capabilities);
    assert(useErrorPortResult.ok, 'Python runtime plan should still be created when capability supports use-error-port.');
    if (!useErrorPortResult.ok) {
        throw new Error('unreachable');
    }
    const useErrorPortArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(useErrorPortResult.plan);
    const useErrorPortFile = join(dir, `use_error_port_${useErrorPortArtifact.filename}`);
    ...
    writeFileSync(useErrorPortRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const useErrorPortCompleted = spawnSync('python', [useErrorPortRunner], { encoding: 'utf8' });
    if (useErrorPortCompleted.error) {
        throw useErrorPortCompleted.error;
    }
    assert(useErrorPortCompleted.status === 0, `Python use-error-port smoke failed with exit ${String(useErrorPortCompleted.status)}\n${useErrorPortCompleted.stdout}\n${useErrorPortCompleted.stderr}`);
    const invalidUseErrorPortResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonErrorPolicy(withPythonErrorPort(unit), {
        selector: { luiId: 'adder' },
        onException: 'use-error-port',
    }), capabilities);
    assert(!invalidUseErrorPortResult.ok &&
        invalidUseErrorPortResult.diagnostics.some((diagnostic) => diagnostic.code === 'PY-007'), 'Python runtime planner should reject use-error-port without selector.portKey before lowering.');
    const emitErrorResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonErrorPolicy(unit, {
        selector: { luiId: 'adder' },
        onException: 'emit-error',
    }), capabilities);
    assert(emitErrorResult.ok, 'Python runtime plan should still be created when capability supports emit-error.');
    if (!emitErrorResult.ok) {
        throw new Error('unreachable');
    }
    const emitErrorArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(emitErrorResult.plan);
    const emitErrorFile = join(dir, `emit_error_${emitErrorArtifact.filename}`);
    writeFileSync(emitErrorFile, emitErrorArtifact.content, 'utf8');
    const emitErrorRunner = join(dir, 'run_emit_error_smoke.py');
    writeFileSync(emitErrorRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const emitErrorCompleted = spawnSync('python', [emitErrorRunner], { encoding: 'utf8' });
    if (emitErrorCompleted.error) {
        throw emitErrorCompleted.error;
    }
    assert(emitErrorCompleted.status === 0, `Python emit-error smoke failed with exit ${String(emitErrorCompleted.status)}\n${emitErrorCompleted.stdout}\n${emitErrorCompleted.stderr}`);
    const unsupportedErrorResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonErrorPolicy(unit, {
        selector: { luiId: 'adder' },
        onException: 'return-exception',
    }), capabilities);
    assert(!unsupportedErrorResult.ok &&
        unsupportedErrorResult.diagnostics.some((diagnostic) => diagnostic.code === 'PY-001'), 'Python runtime planner should reject required return-exception error policy before lowering.');
    const contextManagerResourceResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonResourceLifecyclePolicy(unit, {
        selector: { luiId: 'adder' },
        protocol: 'context-manager',
    }), capabilities);
    assert(contextManagerResourceResult.ok, 'Python runtime plan should still be created when capability supports context-manager resource lifecycle.');
    if (!contextManagerResourceResult.ok) {
        throw new Error('unreachable');
    }
    const contextManagerResourceArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(contextManagerResourceResult.plan);
    const contextManagerResourceFile = join(dir, `context_manager_resource_${contextManagerResourceArtifact.filename}`);
    writeFileSync(contextManagerResourceFile, contextManagerResourceArtifact.content, 'utf8');
    const contextManagerResourceRunner = join(dir, 'run_context_manager_resource_smoke.py');
    writeFileSync(contextManagerResourceRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const contextManagerResourceCompleted = spawnSync('python', [contextManagerResourceRunner], { encoding: 'utf8' });
    if (contextManagerResourceCompleted.error) {
        throw contextManagerResourceCompleted.error;
    }
    assert(contextManagerResourceCompleted.status === 0, `Python context-manager resource smoke failed with exit ${String(contextManagerResourceCompleted.status)}\n${contextManagerResourceCompleted.stdout}\n${contextManagerResourceCompleted.stderr}`);
    const asyncContextManagerResourceResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonResourceLifecyclePolicy(unit, {
        selector: { luiId: 'adder' },
        protocol: 'async-context-manager',
    }), capabilities);
    assert(asyncContextManagerResourceResult.ok, 'Python runtime plan should still be created when capability supports async-context-manager resource lifecycle.');
    if (!asyncContextManagerResourceResult.ok) {
        throw new Error('unreachable');
    }
    const asyncContextManagerResourceArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(asyncContextManagerResourceResult.plan);
    const asyncContextManagerResourceFile = join(dir, `async_context_manager_resource_${asyncContextManagerResourceArtifact.filename}`);
    writeFileSync(asyncContextManagerResourceFile, asyncContextManagerResourceArtifact.content, 'utf8');
    const asyncContextManagerResourceRunner = join(dir, 'run_async_context_manager_resource_smoke.py');
    writeFileSync(asyncContextManagerResourceRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const asyncContextManagerResourceCompleted = spawnSync('python', [asyncContextManagerResourceRunner], { encoding: 'utf8' });
    if (asyncContextManagerResourceCompleted.error) {
        throw asyncContextManagerResourceCompleted.error;
    }
    assert(asyncContextManagerResourceCompleted.status === 0, `Python async-context-manager resource smoke failed with exit ${String(asyncContextManagerResourceCompleted.status)}\n${asyncContextManagerResourceCompleted.stdout}\n${asyncContextManagerResourceCompleted.stderr}`);
    const startStopResourceResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonResourceLifecyclePolicy(unit, {
        selector: { luiId: 'adder' },
        protocol: 'start-stop',
    }), capabilities);
    assert(startStopResourceResult.ok, 'Python runtime plan should still be created when capability supports start-stop resource lifecycle.');
    if (!startStopResourceResult.ok) {
        throw new Error('unreachable');
    }
    const startStopResourceArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(startStopResourceResult.plan);
    const startStopResourceFile = join(dir, `start_stop_resource_${startStopResourceArtifact.filename}`);
    writeFileSync(startStopResourceFile, startStopResourceArtifact.content, 'utf8');
    const startStopResourceRunner = join(dir, 'run_start_stop_resource_smoke.py');
    writeFileSync(startStopResourceRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const startStopResourceCompleted = spawnSync('python', [startStopResourceRunner], { encoding: 'utf8' });
    if (startStopResourceCompleted.error) {
        throw startStopResourceCompleted.error;
    }
    assert(startStopResourceCompleted.status === 0, `Python start-stop resource smoke failed with exit ${String(startStopResourceCompleted.status)}\n${startStopResourceCompleted.stdout}\n${startStopResourceCompleted.stderr}`);
    const asyncStartStopResourceResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonResourceLifecyclePolicy(unit, {
        selector: { luiId: 'adder' },
        protocol: 'start-stop',
    }), capabilities);
    assert(asyncStartStopResourceResult.ok, 'Python runtime plan should support async start-stop resource lifecycle under the start-stop protocol.');
    if (!asyncStartStopResourceResult.ok) {
        throw new Error('unreachable');
    }
    const asyncStartStopResourceArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(asyncStartStopResourceResult.plan);
    const asyncStartStopResourceFile = join(dir, `async_start_stop_resource_${asyncStartStopResourceArtifact.filename}`);
    writeFileSync(asyncStartStopResourceFile, asyncStartStopResourceArtifact.content, 'utf8');
    const asyncStartStopResourceRunner = join(dir, 'run_async_start_stop_resource_smoke.py');
    writeFileSync(asyncStartStopResourceRunner, [
        'import importlib.util',
        'import pathlib',
        '',
    ...
    const asyncStartStopResourceCompleted = spawnSync('python', [asyncStartStopResourceRunner], { encoding: 'utf8' });
    if (asyncStartStopResourceCompleted.error) {
        throw asyncStartStopResourceCompleted.error;
    }
    assert(asyncStartStopResourceCompleted.status === 0, `Python async start-stop resource smoke failed with exit ${String(asyncStartStopResourceCompleted.status)}\n${asyncStartStopResourceCompleted.stdout}\n${asyncStartStopResourceCompleted.stderr}`);
    const unsupportedResourceResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonResourceLifecyclePolicy(unit, {
        selector: { luiId: 'adder' },
        protocol: 'custom',
    }), capabilities);
    assert(!unsupportedResourceResult.ok &&
        unsupportedResourceResult.diagnostics.some((diagnostic) => diagnostic.code === 'PY-005'), 'Python runtime planner should reject required custom resource lifecycle before lowering.');
    const threadConcurrencyResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonConcurrencyPolicy(unit, {
        selector: { luiId: 'adder' },
        execution: 'thread',
        backpressure: 'block',
        ordering: 'preserve',
    }), capabilities);
    assert(threadConcurrencyResult.ok, 'Python runtime plan should still be created when capability supports thread concurrency.');
    if (!threadConcurrencyResult.ok) {
        throw new Error('unreachable');
    }
    const threadConcurrencyArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(threadConcurrencyResult.plan);
    const threadConcurrencyFile = join(dir, `thread_concurrency_${threadConcurrencyArtifact.filename}`);
    writeFileSync(threadConcurrencyFile, threadConcurrencyArtifact.content, 'utf8');
    const threadConcurrencyRunner = join(dir, 'run_thread_concurrency_smoke.py');
    writeFileSync(threadConcurrencyRunner, [
        'import importlib.util',
        'import pathlib',
        'import threading',
        '',
    ...
    const threadConcurrencyCompleted = spawnSync('python', [threadConcurrencyRunner], { encoding: 'utf8' });
    if (threadConcurrencyCompleted.error) {
        throw threadConcurrencyCompleted.error;
    }
    assert(threadConcurrencyCompleted.status === 0, `Python thread concurrency smoke failed with exit ${String(threadConcurrencyCompleted.status)}\n${threadConcurrencyCompleted.stdout}\n${threadConcurrencyCompleted.stderr}`);
    const unsupportedConcurrencyResult = (0, target_plans_1.projectPythonRuntimePlan)(withPythonConcurrencyPolicy(unit, {
        selector: { luiId: 'adder' },
        execution: 'asyncio-task',
        backpressure: 'latest',
    }), capabilities);
    assert(!unsupportedConcurrencyResult.ok &&
        unsupportedConcurrencyResult.diagnostics.some((diagnostic) => diagnostic.code === 'PY-006'), 'Python runtime planner should reject required asyncio-task concurrency before lowering.');
    const statefulUnit = withStatefulCounterPayloadTypes(examples_1.statefulRetainedCounterExample);
    const statefulResult = (0, target_plans_1.projectPythonRuntimePlan)(statefulUnit, capabilities);
    assert(statefulResult.ok, 'Python runtime plan should support typed stateful retained-current counter.');
    if (!statefulResult.ok) {
        throw new Error('unreachable');
    }
    const statefulArtifact = (0, lowering_1.lowerPythonExecutableRuntime)(statefulResult.plan);
    ...
    assert(statefulCompleted.status === 0, `Python stateful smoke failed with exit ${String(statefulCompleted.status)}\n${statefulCompleted.stdout}\n${statefulCompleted.stderr}`);
}
finally {
...
function withPythonLUIRetainedCurrentPolicy(unit, luiId, portKey, payload) {
...
function withPythonErrorPort(unit) {
    const luis = unit.core.luis;
    const adder = luis.adder;
