...
import type {
  ...
  TypeSystemCapability,
  VerilogHDLCapability,
...
type DraftExtensionValidationOptions = {
...
type RequirementSurfaceLike = RequirementSurface | PlainRequirementSurface;
type RequirementServiceLike = RequirementService | PlainRequirementService;
...
type ClassifiedExtension =
  | { family: 'type-system'; key: TypeSystemExtensionKey }
  | { family: 'js-runtime'; key: JSRuntimeExtensionKey }
  | { family: 'python-runtime'; key: PythonRuntimeExtensionKey }
  | { family: 'verilog-hdl'; key: VerilogHDLExtensionKey };
...
type TypeSystemExtensionKey = (typeof TYPE_SYSTEM_KEYS)[number];
...
type JSRuntimeExtensionKey = (typeof JS_RUNTIME_KEYS)[number];
...
type PythonRuntimeExtensionKey = (typeof PYTHON_RUNTIME_KEYS)[number];
...
type VerilogHDLExtensionKey = (typeof VERILOG_HDL_KEYS)[number];
...
const JS_INVOCATIONS = ['sync', 'promise', 'async-iterator'] as const;
const JS_RETAINED = [
  'source-store',
  'sink-cache',
  'projector-adapter',
...
const JS_HOOKS = ['mount', 'start', 'stop', 'dispose'] as const;
...
export function validateDraftExtensions(
  unit: LogicUnit,
  options: DraftExtensionValidationOptions = {}
): ProjectionDiagnostic[] {
...
type TypeRegistry = Map<string, { type: unknown; path: (string | number)[] }>;

type CompositionTypeEntry = {
  type: unknown;
  accepts: 'exact' | 'assignable' | 'custom';
  path: (string | number)[];
};

type StructuralCompositionTypes = {
  exportAnchors: Map<string, CompositionTypeEntry>;
  externalOutlets: Map<string, CompositionTypeEntry>;
  childSurfaces: Map<
    string,
    {
      anchors: Map<string, CompositionTypeEntry>;
      outlets: Map<string, CompositionTypeEntry>;
    }
  >;
};

type CompatibilityPolicy =
  (typeof COMPATIBILITY_POLICIES)[number];

type RequirementCompatibilityRelation =
  | 'same-contract'
  | 'structural-subtype'
  | 'nominal-implements'
  | 'adapter-required';
...
type TypeResolution =
  | { kind: 'missing' }
  | { kind: 'resolved'; type: unknown }
  | { kind: 'invalid-path'; typePath: (string | number)[] }
  | { kind: 'unresolved-named'; typeName: string }
  | { kind: 'recursive-named'; typeName: string };

function validateTypeSystemTypeUsage(
  unit: LogicUnit,
...
function validateCoreTypeUsage(
  core: LUCore,
  path: (string | number)[],
  registry: TypeRegistry,
  sink: DiagnosticSink
  ...
  collectPayloadTypesFromExtensions(
    core.extensions,
    {
      ownerKey: 'lu',
      ports: core.ports,
  ...
  for (const [luiId, lui] of Object.entries(core.luis) as [string, LUI][]) {
    ...
    collectPayloadTypesFromPorts(
      lui.ports,
      ownerKey,
      [...path, 'luis', luiId, 'ports'],
      types,
      sink
    );
  }
  ...
  for (const [closureId, closure] of Object.entries(core.closures)) {
    ...
    collectPayloadTypesFromPorts(
      closure.core.ports,
      ownerKey,
      [...path, 'closures', closureId, 'core', 'ports'],
      types,
      sink
    );
  }
  ...
  for (const [connectionId, connection] of Object.entries(core.connections)) {
    validateConnectionPayloadTypes(
      connectionId,
      connection.from,
      connection.to,
      core,
  ...
  walkLogicUnitExtensions(unit, (extension, _context, path) => {
    ...
    for (const [definitionKey, definition] of Object.entries(
      ...
      if (registry.has(registryKey)) {
        sink.add(
          'EXT-124',
          [...path, 'payload', 'definitions', definitionKey],
          `Duplicate named type definition '${registryKey}'.`
        );
        continue;
      }
...
function validateTypeSystemCompositionUsage(
  unit: LogicUnit,
  registry: TypeRegistry,
  sink: DiagnosticSink
): void {
...
function validateTypeSystemRequirementUsage(
  unit: LogicUnit,
  registry: TypeRegistry,
  options: DraftExtensionValidationOptions,
  sink: DiagnosticSink
): void {
  validateCoreRequirementUsage(
    unit.core,
    unit.requirements,
    ['core'],
    registry,
...
function validateCoreRequirementUsage(
  core: LUCore,
  ...
  for (const [luiId, lui] of Object.entries(core.luis) as [string, LUI][]) {
    const targetRequirements = resolveLUITargetRequirements(
      lui,
      localRequirements,
      options
    );
    ...
    for (const [serviceKey, fulfillment] of Object.entries(lui.fulfillments)) {
      ...
      if (fulfillment.kind === 'independent-units') {
        for (const [unitKey, unitFulfillment] of Object.entries(fulfillment.units)) {
          ...
          const fulfillmentPorts =
            unitFulfillment.kind === 'closure'
              ? core.closures[unitFulfillment.closureId]?.core.ports
              : resolveUpstreamUnitPorts(
                  unitFulfillment.supplierServiceKey,
                  unitFulfillment.supplierUnitKey,
                  localRequirements,
                  options
                );
          if (!fulfillmentPorts) {
          ...
          const relation = selectRequirementCompatibilityRelation(
            requirementUnit,
            service,
            fulfillment,
            unitFulfillment
          );
          validateRequirementUnitFulfillmentCompatibility(
            requirementUnit,
            fulfillmentPorts,
            relation,
            registry,
      ...
      const supplierService = resolveUpstreamService(
        fulfillment.supplier.supplierServiceKey,
        localRequirements,
        options
      );
      if (!supplierService) {
...
function validateRequirementUnitFulfillmentCompatibility(
  requirementUnit: RequirementUnit | PlainRequirementUnit,
  fulfillmentPorts: PortSurface,
  relation: RequirementCompatibilityRelation,
  registry: TypeRegistry,
  path: (string | number)[],
  sink: DiagnosticSink
): void {
  ...
  for (const [portKey, requirementPort] of Object.entries(requirementUnit.ports)) {
    ...
    for (const requirementType of requirementTypes.get(portKey) ?? []) {
      const fulfillmentResolution = resolveSurfaceTypeEntry(
        fulfillmentTypes.get(portKey) ?? [],
        requirementType.payloadPath,
        registry
      );
      ...
      if (fulfillmentResolution.resolution.kind === 'invalid-path') {
        sink.add(
          'EXT-136',
          [...path, 'ports', portKey],
          `Requirement fulfillment port '${portKey}' payload path '${formatPayloadPath(requirementType.payloadPath)}' cannot be resolved against declared payload type.`
        );
        continue;
      }
      const requirementResolution = resolveTypeEntryAtPayloadPath(
        requirementType,
        requirementType.payloadPath,
        registry
      );
      const source =
        requirementPort.boundary === 'input'
      ...
      const target =
        requirementPort.boundary === 'input'
          ? requirementResolution
          : fulfillmentResolution.resolution;
      reportTypeResolution(
        source,
        requirementPort.boundary === 'input'
          ? fulfillmentResolution.path
          : requirementType.path,
        'Requirement fulfillment port',
        sink
      );
      reportTypeResolution(
        target,
        requirementType.path,
        'Requirement contract port',
        sink
      );
      ...
      if (!isTypeCompatible(source.type, target.type, policy, registry)) {
        sink.add(
          'EXT-135',
          [...path, 'ports', portKey],
          `Requirement fulfillment port '${portKey}' payload path '${formatPayloadPath(requirementType.payloadPath)}' is not compatible with requirement contract under '${policy}' policy.`
        );
      }
    }
  }
...
function selectRequirementCompatibilityRelation(
  requirementUnit: RequirementUnit | PlainRequirementUnit,
  service: RequirementServiceLike,
  serviceFulfillment: RequirementServiceFulfillment,
  unitFulfillment: UnitFulfillment | undefined
): RequirementCompatibilityRelation {
  return (
    findRequirementCompatibilityRelation(unitFulfillment?.extensions) ??
...
function findRequirementCompatibilityRelation(
  extensions: ExtensionRecord[] | undefined
): RequirementCompatibilityRelation | undefined {
...
function isRequirementCompatibilityRelation(
  value: unknown
): value is RequirementCompatibilityRelation {
  return (
    value === 'same-contract' ||
...
function requirementTypePolicy(
  requirementPort: Port,
  relation: RequirementCompatibilityRelation
): CompatibilityPolicy {
...
function resolveUpstreamUnitPorts(
  serviceKey: string,
  unitKey: string,
  localRequirements: RequirementSurfaceLike,
  options: DraftExtensionValidationOptions
...
function resolveUpstreamService(
  serviceKey: string,
  localRequirements: RequirementSurfaceLike,
  options: DraftExtensionValidationOptions
): RequirementServiceLike | undefined {
  return resolveRequirementEntryForDraft(localRequirements[serviceKey], options);
}

function collectPayloadTypesForPortSurface(
  ports: PortSurface
): Map<string, TypeEntry[]> {
  const types = new Map<string, TypeEntry[]>();
...
function resolveSurfaceTypeEntry(
  entries: TypeEntry[],
  payloadPath: (string | number)[],
  registry: TypeRegistry
): { kind: 'missing' } | {
...
function resolveTypeEntryAtPayloadPath(
  entry: TypeEntry,
  payloadPath: (string | number)[],
  registry: TypeRegistry
): TypeResolution {
  ...
  const resolved = resolveTypeAtPayloadPath(
    resolvedRoot.type,
    remainingPath,
    registry
  );
...
function formatPayloadPath(payloadPath: (string | number)[]): string {
  return payloadPath.length === 0
    ? '<whole-port>'
    : payloadPath.map(String).join('.');
}

function resolveLUITargetRequirements(
  lui: LUI,
  localRequirements: RequirementSurfaceLike,
  options: DraftExtensionValidationOptions
): RequirementSurfaceLike | undefined {
  if (lui.target.kind === 'requirement') {
    const service = resolveRequirementEntryForDraft(
      localRequirements[lui.target.serviceKey],
      options
    );
...
function resolveRequirementEntryForDraft(
...
function validateCoreCompositionUsage(
  core: LUCore,
  path: (string | number)[],
  registry: TypeRegistry,
  sink: DiagnosticSink
  ...
  if (isStructuralCore(core)) {
    const compositionTypes = collectStructuralCompositionTypes(
      core,
      path,
      sink
    );
    for (const [anchorKey, leaf] of Object.entries(
      ...
      validateCompositionLeafType(
        leaf,
        compositionTypes.exportAnchors.get(anchorKey),
        compositionTypes,
        registry,
    ...
    for (const [luiId, fills] of Object.entries(core.kindOrganization.luiFills)) {
      ...
      for (const [anchorKey, value] of Object.entries(fills)) {
        validateCompositionValueType(
          value,
          childSurface?.anchors.get(anchorKey),
          compositionTypes,
          registry,
          [...path, 'kindOrganization', 'luiFills', luiId, anchorKey],
          sink
        );
      }
    }
...
function collectStructuralCompositionTypes(
  core: StructuralLUCore,
  path: (string | number)[],
  sink: DiagnosticSink
): StructuralCompositionTypes {
  const types: StructuralCompositionTypes = {
    exportAnchors: new Map(),
    externalOutlets: new Map(),
    childSurfaces: new Map(),
  };

  collectCompositionCompatibilityEntries(
    core.extensions,
    {
      anchors: Object.fromEntries(
        Object.keys(core.kindOrganization.exportAnchors).map((key) => [
          key,
          true,
        ])
      ),
      outlets: Object.fromEntries(
        Object.keys(core.kindOrganization.externalOutlets).map((key) => [
          key,
          true,
        ])
      ),
  ...
  for (const [luiId, lui] of Object.entries(core.luis) as [string, LUI][]) {
    ...
    const surfaceTypes = {
      anchors: new Map<string, CompositionTypeEntry>(),
      outlets: new Map<string, CompositionTypeEntry>(),
    };
    collectCompositionCompatibilityEntries(
      lui.compositionSurface.extensions,
      {
        anchors: lui.compositionSurface.anchors,
        outlets: Object.fromEntries(
          lui.compositionSurface.outlets.map((key) => [key, true])
        ),
      },
      surfaceTypes,
      [
...
function collectCompositionCompatibilityEntries(
  extensions: ExtensionRecord[] | undefined,
  surface: {
  ...
  extensions?.forEach((extension, index) => {
    ...
    const selector = extension.payload.selector;
    const accepts = isOneOf(
      extension.payload.accepts,
      ['exact', 'assignable', 'custom'] as const
    )
      ? extension.payload.accepts
      : 'exact';
    ...
    if (
      typeof selector.anchorKey === 'string' &&
      hasOwn(surface.anchors, selector.anchorKey)
    ) {
      if (target.anchors.has(selector.anchorKey)) {
        sink.add(
          'EXT-132',
          [...path, index, 'payload', 'selector', 'anchorKey'],
          `Duplicate composition type for anchor '${selector.anchorKey}'.`
        );
      } else {
    ...
    if (
      typeof selector.outletKey === 'string' &&
      hasOwn(surface.outlets, selector.outletKey)
    ) {
      if (target.outlets.has(selector.outletKey)) {
        sink.add(
          'EXT-132',
          [...path, index, 'payload', 'selector', 'outletKey'],
          `Duplicate composition type for outlet '${selector.outletKey}'.`
        );
      } else {
...
function validateCompositionValueType(
  value: CompositionValue,
  target: CompositionTypeEntry | undefined,
  types: StructuralCompositionTypes,
  registry: TypeRegistry,
  path: (string | number)[],
  sink: DiagnosticSink
): void {
...
function validateCompositionLeafType(
  ...
  if (!source) {
    sink.add(
      'EXT-131',
      path,
      'Composition fill targets a typed anchor, but the source outlet has no declared composition type.'
    );
    return;
  }
  ...
  if (
    ...
    sink.add(
      'EXT-134',
      path,
      `Composition outlet type is not compatible with anchor under '${target.accepts}' policy.`
    );
  }
}

function resolveCompositionLeafType(
  leaf: CompositionLeaf,
  types: StructuralCompositionTypes
): CompositionTypeEntry | undefined {
...
function reportTypeResolution(
  ...
  if (resolution.kind === 'unresolved-named') {
    sink.add(
      'EXT-133',
      path,
      `${label} references unknown named type '${resolution.typeName}'.`
    );
  }
  if (resolution.kind === 'recursive-named') {
    sink.add(
      'EXT-133',
      path,
      `${label} named type '${resolution.typeName}' is recursive.`
    );
  }
}

function collectPayloadTypesFromPorts(
  ports: PortSurface,
  ownerKey: string,
  path: (string | number)[],
...
function collectPayloadTypesFromExtensions(
  extensions: ExtensionRecord[] | undefined,
  context: {
    ownerKey: string;
    ports: PortSurface;
    currentPortKey: string | undefined;
    path: (string | number)[];
  },
  types: Map<string, TypeEntry[]>,
  sink: DiagnosticSink
  ...
  extensions?.forEach((extension, index) => {
    ...
    const selectedPortKey =
      isRecord(selector) && typeof selector.portKey === 'string'
        ? selector.portKey
    ...
    if (
      entries.some((entry) => samePath(entry.payloadPath, selectedPath))
    ) {
      sink.add(
        'EXT-121',
        [...context.path, index],
        `Duplicate payload type for port '${selectedPortKey}' at the same payload path.`
      );
      return;
    }
...
function validateConnectionPayloadTypes(
  connectionId: string,
  from: EndpointRef,
  to: EndpointRef,
  core: LUCore,
  ...
  if (fromType.kind === 'invalid-path') {
    sink.add(
      ...
      'Connection source payloadPath cannot be resolved against declared payload type.'
    );
  }
  if (toType.kind === 'invalid-path') {
    sink.add(
      'EXT-122',
      [...path, 'to', 'payloadPath'],
      'Connection target payloadPath cannot be resolved against declared payload type.'
    );
  }
  if (fromType.kind === 'unresolved-named') {
    sink.add(
      'EXT-125',
      [...path, 'from'],
      `Connection source references unknown named type '${fromType.typeName}'.`
    );
  }
  if (toType.kind === 'unresolved-named') {
    sink.add(
      'EXT-125',
      [...path, 'to'],
      `Connection target references unknown named type '${toType.typeName}'.`
    );
  }
  if (fromType.kind === 'recursive-named') {
    sink.add(
      'EXT-126',
      [...path, 'from'],
      `Connection source named type '${fromType.typeName}' is recursive.`
    );
  }
  if (toType.kind === 'recursive-named') {
    sink.add(
      'EXT-126',
      [...path, 'to'],
      `Connection target named type '${toType.typeName}' is recursive.`
    );
  }
  ...
  const policy = resolveConnectionCompatibilityPolicy(
    connectionId,
    core,
    path
  );
  ...
  if (!isTypeCompatible(fromType.type, toType.type, policy, registry)) {
    sink.add(
      'EXT-123',
      path,
      `Connection payload types are not compatible under '${policy}' policy.`
    );
  }
}
...
function resolveConnectionCompatibilityPolicy(
  connectionId: string,
  core: LUCore,
  path: (string | number)[]
): CompatibilityPolicy {
  const connection = core.connections[connectionId];
  const connectionPolicy = firstCompatibilityPolicy(
  ...
  const corePolicy = firstCompatibilityPolicy(
    core.extensions,
    connectionId,
    connection
  );
...
function firstCompatibilityPolicy(
  extensions: ExtensionRecord[] | undefined,
  connectionId: string,
  connection: { from: EndpointRef; to: EndpointRef } | undefined
...
function selectorMatchesConnection(
  ...
  return (
    endpointSelectorMatches(selector.from, connection.from) &&
    endpointSelectorMatches(selector.to, connection.to)
  );
}

function endpointSelectorMatches(
  selector: unknown,
  endpoint: EndpointRef
): boolean {
  if (selector === undefined) {
...
function isTypeWidening(
  ...
  if (
    typeEquals(source, target, registry) ||
  ...
  if (source.kind === 'record' && target.kind === 'record') {
    ...
    return (
      ...
      sourceKeys.every((key) => {
        ...
        return (
          isOptionalTypeField(sourceField) === isOptionalTypeField(targetField) &&
          isTypeWidening(
            typeFieldType(sourceField),
            typeFieldType(targetField),
            registry
          )
        );
      })
  ...
  if (source.kind === 'tuple' && target.kind === 'tuple') {
    ...
    return (
      ...
      sourceItems.every((item, index) =>
        isTypeWidening(item, targetItems[index], registry)
      )
    );
  }
...
function isTypeAssignable(
  ...
  if (isRecord(source) && source.kind === 'union') {
    return (
      ...
      source.variants.every((variant) =>
        isTypeAssignable(variant, target, registry)
      )
    );
  }
  if (isRecord(target) && target.kind === 'union') {
    return (
      ...
      target.variants.some((variant) =>
        isTypeAssignable(source, variant, registry)
      )
    );
  }
  ...
  if (source.kind === 'record' && target.kind === 'record') {
    ...
    return Object.entries(targetFields).every(([fieldKey, targetField]) => {
      ...
      return (
        isTypeAssignable(typeFieldType(sourceField), typeFieldType(targetField), registry)
      );
    });
  }
  ...
  if (source.kind === 'tuple' && target.kind === 'array') {
    ...
    return sourceItems.every((sourceItem) =>
      isTypeAssignable(sourceItem, target.item, registry)
    );
  }
  ...
  if (source.kind === 'array' && target.kind === 'tuple') {
    ...
    return targetItems.every((targetItem) =>
      isTypeAssignable(source.item, targetItem, registry)
    );
  }
  ...
  if (source.kind === 'tuple' && target.kind === 'tuple') {
    ...
    return targetItems.every((targetItem, index) =>
      isTypeAssignable(sourceItems[index], targetItem, registry)
    );
  }
...
function typeEquals(
  ...
  if (left.kind === 'tuple' && right.kind === 'tuple') {
    ...
    return (
      ...
      leftItems.every((item, index) =>
        typeEquals(item, rightItems[index], registry)
      )
    );
  }
  if (left.kind === 'union' && right.kind === 'union') {
    ...
    return (
      ...
      leftVariants.every((leftVariant, index) =>
        typeEquals(leftVariant, rightVariants[index], registry)
      )
    );
  }
  if (left.kind === 'record' && right.kind === 'record') {
    ...
    return (
      ...
      leftKeys.every((key) =>
        isOptionalTypeField(leftFields[key]) === isOptionalTypeField(rightFields[key]) &&
...
function isTypeField(value: unknown): value is { type: unknown; optional?: unknown } {
  return (
    isRecord(value) &&
    hasOwn(value, 'type') &&
    !hasOwn(value, 'kind')
...
function typeFieldType(value: unknown): unknown {
  return isTypeField(value) ? value.type : value;
}

function isOptionalTypeField(value: unknown): boolean {
  return isTypeField(value) && value.optional === true;
}

function resolveTypeAtPayloadPath(
  type: unknown,
  payloadPath: (string | number)[],
  registry: TypeRegistry
): TypeResolution {
...
function stripDefinitionIdentity(type: Record<string, unknown>): unknown {
  const { namespace: _namespace, key: _key, ...rest } = type;
  return rest;
}

function namedTypeKey(namespace: unknown, key: unknown): string {
  return `${String(namespace)}:${String(key)}`;
}

function namespaceKeyType(value: unknown): unknown | undefined {
  if (
    isRecord(value) &&
...
function validateTypeSystemPathSchemaUsage(
  unit: LogicUnit,
  sink: DiagnosticSink
): void {
...
function validateCorePathSchemaUsage(
  core: LUCore,
  path: (string | number)[],
  sink: DiagnosticSink
): void {
  const schemas = new Map<string, { root: unknown; path: (string | number)[] }>();

  collectPathSchemasFromExtensions(
    core.extensions,
    {
      ownerKey: 'lu',
      ports: core.ports,
  ...
  for (const [connectionId, connection] of Object.entries(core.connections)) {
    validateEndpointPathSchema(
      connection.from,
      schemas,
      [...path, 'connections', connectionId, 'from', 'payloadPath'],
      sink
    );
    validateEndpointPathSchema(
      connection.to,
      schemas,
      [...path, 'connections', connectionId, 'to', 'payloadPath'],
      sink
    );
  }

  for (const [closureId, closure] of Object.entries(core.closures)) {
    validateCorePathSchemaUsage(
      closure.core,
      [...path, 'closures', closureId, 'core'],
      sink
    );
...
function collectPathSchemasFromPorts(
  ports: PortSurface,
  ownerKey: string,
  path: (string | number)[],
  schemas: Map<string, { root: unknown; path: (string | number)[] }>,
  sink: DiagnosticSink
): void {
  for (const [portKey, port] of Object.entries(ports)) {
    collectPathSchemasFromExtensions(
...
function collectPathSchemasFromExtensions(
  extensions: ExtensionRecord[] | undefined,
  context: {
    ownerKey: string;
    ports: PortSurface;
  ...
  extensions?.forEach((extension, index) => {
    ...
    if (schemas.has(schemaKey)) {
      sink.add(
        'EXT-119',
        [...context.path, index],
        `Duplicate path schema for port '${selectedPortKey}'.`
      );
      return;
    }
...
function validateEndpointPathSchema(
  endpoint: EndpointRef,
  schemas: Map<string, { root: unknown; path: (string | number)[] }>,
  path: (string | number)[],
  sink: DiagnosticSink
  ...
  validatePayloadPathAgainstPathNode(
    schema.root,
...
function validatePayloadPathAgainstPathNode(
  root: unknown,
  payloadPath: (string | number)[],
  path: (string | number)[],
  sink: DiagnosticSink
    ...
    if (node.kind === 'record') {
      if (
        ...
        sink.add(
          'EXT-120',
          [...path, index],
          `Payload path segment '${String(segment)}' is not a record field.`
        );
        return;
      }
    ...
    if (node.kind === 'tuple') {
      if (
        ...
        sink.add(
          'EXT-120',
          [...path, index],
          `Payload path segment '${String(segment)}' is not a tuple index.`
        );
        return;
      }
    ...
    if (node.kind === 'array') {
      if (typeof segment !== 'number' || !Number.isInteger(segment) || segment < 0) {
        sink.add(
          'EXT-120',
          [...path, index],
          `Payload path segment '${String(segment)}' is not an array index.`
        );
        return;
      }
      node = node.item;
...
function portOwnerSchemaKey(owner: { kind: string; luiId?: string; closureId?: string }): string {
  if (owner.kind === 'lu') {
    return 'lu';
  }
  if (owner.kind === 'lui') {
...
function validateClassifiedExtension(
  extension: ExtensionRecord,
  classified: ClassifiedExtension,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  capabilities: ProjectorCapabilitySet | undefined,
  sink: DiagnosticSink
): void {
  validateTargetSpecificCapability(
    extension,
    classified,
    path,
    capabilities,
...
function validateTargetSpecificCapability(
  extension: ExtensionRecord,
  classified: ClassifiedExtension,
  path: (string | number)[],
  capabilities: ProjectorCapabilitySet | undefined,
  ...
  if (!targetCapability && extension.requirement === 'required') {
    sink.add(
      'EXT-001',
      path,
      `Projector target '${capabilities.target}' does not declare ${classified.family} capability.`
    );
  }
}

function validateTypeSystemPayload(
  extension: ExtensionRecord,
  key: TypeSystemExtensionKey,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  capability: TypeSystemCapability | undefined,
  sink: DiagnosticSink
): void {
  ...
  if (key === 'type-definitions') {
    validateTypeDefinitionsPayload(
      payload,
      [...path, 'payload'],
      capability,
      extension,
  ...
  if (key === 'port-compatibility') {
    validatePortCompatibilityPayload(
      payload,
      context,
      [...path, 'payload'],
      capability,
  ...
  if (key === 'requirement-compatibility') {
    ...
    if (
      ...
      sink.add(
        'EXT-108',
        path,
        'Projector does not support required requirement compatibility.'
      );
    }
    return;
  }
  ...
  if (key === 'composition-compatibility') {
    validateCompositionCompatibilityPayload(
      payload,
      context,
      [...path, 'payload'],
      capability,
...
function validateTypeDefinitionsPayload(
  ...
  for (const [definitionKey, definition] of Object.entries(payload.definitions)) {
    if (!isRecord(definition)) {
      sink.add(
        'EXT-128',
        [...path, 'definitions', definitionKey],
        'Type definition must be a type expression object.'
      );
      continue;
    }
    if (
      ...
      sink.add(
        'EXT-129',
        [...path, 'definitions', definitionKey, 'namespace'],
        'Type definition namespace must be a string.'
      );
    }
    if (definition.key !== undefined && typeof definition.key !== 'string') {
      sink.add(
        'EXT-130',
        [...path, 'definitions', definitionKey, 'key'],
        'Type definition key must be a string.'
      );
    }
    validateTypeExpression(
      stripDefinitionIdentity(definition),
      [...path, 'definitions', definitionKey],
      capability,
      extension,
      sink
...
function validatePortCompatibilityPayload(
  payload: Record<string, unknown>,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  capability: TypeSystemCapability | undefined,
  ...
  validateOptionalPortCompatibilitySelector(
    payload.selector,
    context,
    [...path, 'selector'],
    sink
  ...
  if (
    ...
    sink.add(
      'EXT-103',
      [...path, 'policy'],
      `Projector does not support required compatibility policy '${payload.policy}'.`
    );
  }
  ...
  if (payload.policy === 'projector-adapter' && payload.adapterTarget === undefined) {
    sink.add(
      'EXT-137',
      [...path, 'adapterTarget'],
      'projector-adapter compatibility requires adapterTarget evidence.'
    );
  } else if (payload.policy === 'projector-adapter') {
    validateNamespaceKey(
      payload.adapterTarget,
      [...path, 'adapterTarget'],
      sink
    );
  }
...
function validateRequirementCompatibilityPayload(
  payload: Record<string, unknown>,
  path: (string | number)[],
  sink: DiagnosticSink
): void {
  if (
    !isOneOf(payload.relation, [
      'same-contract',
      'structural-subtype',
      'nominal-implements',
      'adapter-required',
  ...
  if (payload.relation === 'adapter-required' && payload.evidence === undefined) {
    sink.add(
      'EXT-138',
      [...path, 'evidence'],
      'adapter-required requirement compatibility requires evidence.'
    );
  }
...
function validateCompositionCompatibilityPayload(
  payload: Record<string, unknown>,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  capability: TypeSystemCapability | undefined,
  ...
  if (
    ...
    !isOneOf(payload.accepts, ['exact', 'assignable', 'custom'] as const)
  ) {
  ...
  if (
    ...
    sink.add(
      'EXT-106',
      path,
      'Projector does not support required composition compatibility.'
    );
  }
}

function validatePathSchemaPayload(
  payload: Record<string, unknown>,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  capability: TypeSystemCapability | undefined,
  ...
  if (capability && !capability.pathSchema && extension.requirement === 'required') {
    sink.add(
      'EXT-107',
      path,
      'Projector does not support required path schemas.'
    );
  }
}
...
function validateTypeExpression(
  ...
  if (
    ...
    sink.add(
      'EXT-110',
      [...path, 'kind'],
      `Projector does not support required type form '${value.kind}'.`
    );
  }

  if (value.kind === 'primitive') {
    if (!isOneOf(value.name, ['bool', 'int', 'float', 'string'] as const)) {
...
function validatePathNode(
  ...
  if (
    !isRecord(value) ||
    !isOneOf(value.kind, ['leaf', 'record', 'tuple', 'array'] as const)
  ) {
...
function validateJSRuntimePayload(
  extension: ExtensionRecord,
  key: JSRuntimeExtensionKey,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  capability: JSRuntimeCapability | undefined,
  sink: DiagnosticSink
): void {
  const payload = extension.payload;
  if (!isRecord(payload)) {
  ...
  if (key === 'retained-current-realization') {
    ...
    validateCapabilityEnum(
      payload.realization,
      JS_RETAINED,
      capability?.retainedCurrentRealization,
      extension,
      [...path, 'payload', 'realization'],
      'EXT-202',
      'JS retained-current realization',
      sink
    );
    validateOptionalEnum(
      payload.notification,
      ['push', 'subscribe', 'microtask', 'custom'] as const,
      [...path, 'payload', 'notification'],
      'EXT-203',
      sink
    );
    return;
  ...
  if (key === 'dynamic-fulfillment') {
    ...
    validateCapabilityEnum(
      payload.mode,
      JS_DYNAMIC,
      capability?.dynamicFulfillment,
      extension,
      [...path, 'payload', 'mode'],
      'EXT-204',
      'JS dynamic fulfillment mode',
      sink
    );
    validateRequiredEnum(
      payload.consistency,
      ['no-live-switch', 'quiescent-switch', 'transactional-switch'] as const,
      [...path, 'payload', 'consistency'],
      'EXT-205',
      sink
    );
    return;
  ...
  if (key === 'lifecycle') {
    ...
    validateOptionalEnum(
      payload.ordering,
      ['parent-before-child', 'child-before-parent'] as const,
      [...path, 'payload', 'ordering'],
      'EXT-207',
      sink
    );
    return;
  ...
  validateCapabilityEnum(
    payload.onThrow,
    JS_ERROR_POLICIES,
    capability?.errorPolicies,
    extension,
    [...path, 'payload', 'onThrow'],
    'EXT-208',
    'JS error policy',
    sink
  );
  validateOptionalEnum(
    ...
    ['unsupported', 'abort-signal', 'custom'] as const,
    [...path, 'payload', 'cancellation'],
    'EXT-209',
    sink
  );
}

function validatePythonRuntimePayload(
  extension: ExtensionRecord,
  key: PythonRuntimeExtensionKey,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  capability: PythonRuntimeCapability | undefined,
  sink: DiagnosticSink
): void {
  const payload = extension.payload;
  if (!isRecord(payload)) {
  ...
  if (key === 'retained-current-realization') {
    ...
    validateCapabilityEnum(
      ...
      [...path, 'payload', 'realization'],
      'EXT-302',
      'Python retained-current realization',
      sink
    );
    validateOptionalEnum(
      payload.notification,
      ['callback', 'asyncio-event', 'queue', 'poll'] as const,
      [...path, 'payload', 'notification'],
      'EXT-303',
      sink
    );
  ...
  if (key === 'dynamic-fulfillment') {
    ...
    validateCapabilityEnum(
      ...
      capability?.dynamicFulfillment,
      extension,
      [...path, 'payload', 'binding'],
      'EXT-304',
      'Python dynamic fulfillment binding',
      sink
    );
    validateRequiredEnum(
      payload.consistency,
      ['startup-only', 'task-local', 'quiescent-switch', 'transactional-switch'] as const,
      [...path, 'payload', 'consistency'],
      'EXT-305',
      sink
    );
  ...
  if (key === 'concurrency') {
    ...
    validateCapabilityEnum(
      ...
      extension,
      [...path, 'payload', 'execution'],
      'EXT-308',
      'Python concurrency execution',
      sink
    );
    validateOptionalEnum(
      payload.backpressure,
      ['drop', 'latest', 'buffer', 'block', 'custom'] as const,
      [...path, 'payload', 'backpressure'],
      'EXT-309',
      sink
    );
    validateOptionalEnum(
      payload.ordering,
      ['preserve', 'best-effort', 'unordered'] as const,
      [...path, 'payload', 'ordering'],
      'EXT-310',
      sink
    );
  ...
  validateOptionalEnum(
    payload.cancellation,
    ['unsupported', 'asyncio-cancel', 'cooperative', 'custom'] as const,
    [...path, 'payload', 'cancellation'],
    'EXT-312',
    sink
  );
...
function validateVerilogHDLPayload(
  extension: ExtensionRecord,
  key: VerilogHDLExtensionKey,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  capability: VerilogHDLCapability | undefined,
  sink: DiagnosticSink
): void {
  const payload = extension.payload;
  if (!isRecord(payload)) {
  ...
  if (key === 'signal-types') {
    ...
    validateOptionalEnum(
      payload.encoding,
      ['bits', 'one-hot', 'gray', 'custom'] as const,
      [...path, 'payload', 'encoding'],
      'EXT-402',
      sink
    );
    requireHDLCapability(
      capability?.signalTypes,
      extension,
      path,
      'signal-types',
      sink
  ...
  if (key === 'combinational-assigns') {
    ...
    requireHDLCapability(
      capability?.combinationalAssigns,
      extension,
      path,
      'combinational-assigns',
      sink
  ...
  validateStructuralSlicesPayload(
    payload,
    context,
    [...path, 'payload'],
    capability,
...
function validateClockResetPayload(
  payload: Record<string, unknown>,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
...
function validateHDLCombinationalAssignsPayload(
  payload: Record<string, unknown>,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
...
function validateHDLStateRegistersPayload(
  payload: Record<string, unknown>,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
  ...
  payload.registers.forEach((register, index) => {
    ...
    if (
      register.clockResetDomain !== undefined &&
      typeof register.clockResetDomain !== 'string'
    ) {
      sink.add(
        'EXT-434',
        [...registerPath, 'clockResetDomain'],
        'clockResetDomain must be a string.'
      );
    }
  });
}

function validateHDLExpression(
  expr: unknown,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
  ...
  if (expr.kind === 'concat') {
    ...
    expr.items.forEach((item, index) =>
      validateHDLExpression(item, context, [...path, 'items', index], sink)
    );
    return;
  }
  if (expr.kind === 'cast') {
...
function validateHDLEndpoint(
  ...
  if (owner.kind === 'lu') {
    ...
    validateEndpointOwnerPortKey(
      luiId ? context.core?.luis[luiId]?.ports : undefined,
      endpoint.portKey,
      [...path, 'portKey'],
      sink
    );
    ...
    const closureId =
      typeof owner.closureId === 'string' ? owner.closureId : undefined;
    validateEndpointOwnerPortKey(
      closureId ? context.core?.closures[closureId]?.core.ports : undefined,
      endpoint.portKey,
      [...path, 'portKey'],
      sink
    );
  ...
  if (
    endpoint.payloadPath !== undefined &&
...
function validateEndpointOwnerPortKey(
  ports: PortSurface | undefined,
  value: unknown,
  path: (string | number)[],
  sink: DiagnosticSink
): void {
...
function validateStructuralSlicesPayload(
  payload: Record<string, unknown>,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  capability: VerilogHDLCapability | undefined,
  ...
  if (!isPlainObject(payload.slices)) {
    ...
    for (const [anchorKey, slice] of Object.entries(payload.slices)) {
      if (
        context.core?.kindOrganization.kind === 'structural' &&
        !hasOwn(context.core.kindOrganization.exportAnchors, anchorKey)
      ) {
        sink.add(
          'EXT-414',
          [...path, 'slices', anchorKey],
          `Structural slice references unknown export anchor '${anchorKey}'.`
        );
      }
  ...
  if (payload.bus !== undefined) {
    if (!isRecord(payload.bus)) {
      ...
      validateRequiredEnum(
        payload.bus.routing,
        ['payload-path', 'pin-channel', 'custom'] as const,
        [...path, 'bus', 'routing'],
        'EXT-416',
        sink
      );
      if (
        payload.bus.channelPath !== undefined &&
    ...
    if (!isPlainObject(payload.fanIn)) {
      ...
      for (const [anchorKey, fanIn] of Object.entries(payload.fanIn)) {
        if (
          ...
          sink.add(
            'EXT-441',
            [...path, 'fanIn', anchorKey],
            `Structural slice fanIn references unknown export anchor '${anchorKey}'.`
          );
        }
        validateStructuralSliceFanIn(
          fanIn,
          [...path, 'fanIn', anchorKey],
          sink
        );
...
function validateStructuralSlice(
  value: unknown,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
...
function validateStructuralSliceFanIn(
  value: unknown,
  path: (string | number)[],
  sink: DiagnosticSink
): void {
...
function validateOptionalHDLSliceInterface(
  value: unknown,
  path: (string | number)[],
  sink: DiagnosticSink
): void {
...
function validateCapabilityEnum<
...
>(
  ...
  if (
    ...
    sink.add(
      code,
      path,
      `Projector does not support required ${label} '${value}'.`
    );
  }
}
  ...
  value.forEach((hook, index) => {
    ...
    if (
      ...
      sink.add(
        code,
        [...path, index],
        `Projector does not support required lifecycle hook '${hook}'.`
      );
    }
  });
}
...
function validateOptionalPortCompatibilitySelector(
  selector: unknown,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
...
function validateCompositionSelector(
  selector: unknown,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
  ...
  if (selector.anchorKey !== undefined) {
    if (typeof selector.anchorKey !== 'string') {
      ...
      context.compositionSurface &&
      !hasOwn(context.compositionSurface.anchors, selector.anchorKey)
    ) {
      sink.add(
        'EXT-509',
        [...path, 'anchorKey'],
        `Unknown composition anchor '${selector.anchorKey}'.`
      );
    }
  }
  ...
  if (selector.outletKey !== undefined) {
    if (typeof selector.outletKey !== 'string') {
      ...
      sink.add(
        'EXT-511',
        [...path, 'outletKey'],
        `Unknown composition outlet '${selector.outletKey}'.`
      );
    }
  }
}

function validateStepLUISelector(
  selector: unknown,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
...
function validateStepLUIPortSelector(
  selector: unknown,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
...
function validateLUIClosureSelector(
  selector: unknown,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
...
function validateLUIPortSelector(
  selector: unknown,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
...
function validateLUIConnectionSelector(
  selector: unknown,
  context: ExtensionOwnerContext,
  path: (string | number)[],
  sink: DiagnosticSink
...
function validateServiceUnitSelector(
  selector: unknown,
  path: (string | number)[],
  sink: DiagnosticSink
): void {
...
function validateStepIndex(
  ...
  if (
    context.core?.kindOrganization.kind === 'sequential' &&
...
function validateOptionalStringBooleanNumberMap(
  value: unknown,
  path: (string | number)[],
  sink: DiagnosticSink
): void {
...
function walkLogicUnitExtensions(
  unit: LogicUnit,
  visit: (
    extension: ExtensionRecord,
    context: ExtensionOwnerContext,
...
function walkCore(
  ...
  visitExtensionList(
    core.extensions,
    { unit, core, ports: core.ports, path },
    [...path, 'extensions'],
    visit
  );
  ...
  for (const [connectionId, connection] of Object.entries(core.connections)) {
    visitExtensionList(
      connection.extensions,
      { unit, core, connectionId, path: [...path, 'connections', connectionId] },
      [...path, 'connections', connectionId, 'extensions'],
      visit
    );
  }
  ...
  for (const [closureId, closure] of Object.entries(core.closures)) {
    ...
    visitExtensionList(
      closure.extensions,
      { unit, core, path: closurePath },
      [...closurePath, 'extensions'],
      visit
    );
...
function walkLUI(
  ...
  const compositionSurface =
    lui.kind === 'structural' ? lui.compositionSurface : undefined;
  visitExtensionList(
    lui.extensions,
    { unit, core, ports: lui.ports, compositionSurface, path },
    [...path, 'extensions'],
    visit
  );
  ...
  if (lui.kind === 'structural') {
    visitExtensionList(
      ...
      [...path, 'compositionSurface', 'extensions'],
      visit
    );
  }

  for (const [serviceKey, fulfillment] of Object.entries(lui.fulfillments)) {
    walkRequirementServiceFulfillment(
      fulfillment,
      unit,
      core,
...
function walkPorts(
  ...
  for (const [portKey, port] of Object.entries(ports)) {
    visitExtensionList(
      port.extensions,
      { ...context, ports, portKey, path: [...path, portKey] },
      [...path, portKey, 'extensions'],
      visit
    );
  }
}

function walkRequirementSurface(
...
function walkRequirementService(
  service: RequirementServiceLike,
  unit: LogicUnit,
  path: (string | number)[],
  visit: (
  ...
  for (const [unitKey, requirementUnit] of Object.entries(service.units)) {
    ...
    walkPorts(
      requirementUnit.ports,
      { unit, ports: requirementUnit.ports, path: unitPath },
      [...unitPath, 'ports'],
      visit
    );
    if (hasNestedRequirementSurface(requirementUnit)) {
      walkRequirementSurface(
        requirementUnit.requirements,
        unit,
    ...
    if (requirementUnit.kind === 'structural') {
      visitExtensionList(
        requirementUnit.compositionSurface.extensions,
        {
          unit,
        ...
        [...unitPath, 'compositionSurface', 'extensions'],
        visit
      );
    }
  }
...
function walkRequirementServiceFulfillment(
...
function resolveKnownKey<T extends readonly string[]>(
  extension: ExtensionRecord,
  keys: T
): T[number] | undefined {
...
function isPayloadPath(value: unknown): value is (string | number)[] {
  return (
    Array.isArray(value) &&
    value.every(
      (segment) =>
...
function selectorPayloadPath(selector: unknown): (string | number)[] {
  if (
    isRecord(selector) &&
    selector.payloadPath !== undefined &&
    isPayloadPath(selector.payloadPath)
...
function isValidPathNode(value: unknown): boolean {
  if (
    !isRecord(value) ||
    !isOneOf(value.kind, ['leaf', 'record', 'tuple', 'array'] as const)
  ) {
...
function isValidTypeExpression(value: unknown): boolean {
  if (!isRecord(value) || !isOneOf(value.kind, TYPE_FORMS)) {
    return false;
  }
  if (value.kind === 'primitive') {
    return isOneOf(value.name, ['bool', 'int', 'float', 'string'] as const);
  }
  if (value.kind === 'named') {
    return (
      typeof value.namespace === 'string' &&
  ...
  if (value.kind === 'array') {
    const length = value.length;
    return (
      isValidTypeExpression(value.item) &&
      (length === undefined ||
...
function isValidTypeField(value: unknown): boolean {
  if (isTypeField(value)) {
    return (
      isValidTypeExpression(value.type) &&
...
function hasNestedRequirementSurface(
  unit: unknown
): unit is { requirements: RequirementSurfaceLike } {
...
function isCompositionLeaf(value: CompositionValue): value is CompositionLeaf {
  return (
